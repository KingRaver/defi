...
from utils.bot_monitor import BotMethodMonitor, setup_bot_monitoring
import anthropic
...
class DataFetchCircuitBreaker:
    """Circuit breaker specifically for preventing infinite data fetch loops"""
    
    def __init__(self):
        self.failure_count = 0
        self.last_failure_time = None
        self.circuit_open_until = None
        self.max_failures = 3  # Open circuit after 3 consecutive failures
        self.circuit_timeout = 300  # Keep circuit open for 5 minutes
        self.failure_window = 120  # 2-minute window for counting failures
        self.last_successful_fetch = None
    
    def can_attempt_fetch(self) -> bool:
        """Check if we can attempt to fetch data"""
        now = time.time()
        
        # Circuit is open (blocked)
        if self.circuit_open_until and self.circuit_open_until > now:
            remaining_time = int(self.circuit_open_until - now)
            logger.logger.debug(f"🛡️ Circuit breaker OPEN - {remaining_time}s remaining")
            return False
        
        # Circuit timeout expired, reset
        if self.circuit_open_until and self.circuit_open_until <= now:
            self._reset_circuit()
            logger.logger.info("🛡️ Circuit breaker RESET - allowing data fetch attempt")
    ...
    def record_failure(self, error_type: str = "generic"):
        ...
        # Reset failure count if too much time has passed since last failure
        if (self.last_failure_time and 
            now - self.last_failure_time > self.failure_window):
            self.failure_count = 0
        
        self.failure_count += 1
        self.last_failure_time = now
        
        logger.logger.warning(f"🛡️ Circuit breaker failure #{self.failure_count}: {error_type}")
    ...
    def record_success(self):
        ...
        self._reset_circuit()
        logger.logger.debug("🛡️ Circuit breaker: Successful data fetch recorded")
    
    def _open_circuit(self):
        """Open circuit (block requests)"""
        now = time.time()
        self.circuit_open_until = now + self.circuit_timeout
        logger.logger.error(f"🛡️ CIRCUIT BREAKER OPENED: Blocking data fetches for {self.circuit_timeout/60:.1f} minutes")
        logger.logger.error("🛡️ This prevents infinite API loops and 429 rate limit errors")
...
class CryptoAnalysisBot:
    ...
    def __init__(self, database, llm_provider, config=None):
        ...
        self._initialize_token_mapper()

        # 🛡️ CIRCUIT BREAKER: Prevent infinite API loops
        self.data_fetch_circuit_breaker = DataFetchCircuitBreaker()
        ...
        try:
            ...
            if not database_tokens:
                ...
                logger.logger.info("🔄 Bot will continue running and gather fresh data")
                database_tokens = []  # Continue with empty list instead of shutting down
            ...
            logger.logger.warning(f"⚠️ Database access failed: {str(e)} - continuing with empty token list")
            self.target_chains = {}
        ...
        self.content_analyzer = ContentAnalyzer(self.config, self.db, self.llm_provider)
    ...
    def _initialize_token_mapper(self):
        """
        Initialize shared TokenMappingManager for the bot
        
        Note: TokenMappingManager creates its own database connection internally
        through CryptoDatabase() in its methods, so we don't need to assign db
        """
        try:
            from config import TokenMappingManager
            self.token_mapper = TokenMappingManager()
            logger.logger.info("✅ TokenMappingManager initialized successfully")
            
            # Test if the token mapper is working properly
            try:
                # Try a simple lookup to verify it's functional
                test_result = self.token_mapper.symbol_to_coingecko_id('BTC')
                if test_result:
                    logger.logger.debug(f"✅ TokenMappingManager test successful: BTC -> {test_result}")
                else:
                    logger.logger.warning("⚠️ TokenMappingManager test returned no result for BTC")
            except Exception as test_error:
                logger.logger.warning(f"⚠️ TokenMappingManager test failed: {str(test_error)}")
                
        except Exception as token_mapper_error:
            logger.logger.error(f"❌ TokenMappingManager initialization failed: {str(token_mapper_error)}")
    ...
    def _standardize_market_data(self, market_data):
        """
        Enhanced market data standardization using database-aware TokenMappingManager
        Supports 150+ tokens across database, CoinGecko, and CoinMarketCap formats
        Uses database-first approach with intelligent fallback hierarchy
        """
        logger.logger.info(f"🔍 BOT._standardize_market_data() CALLED - Input type: {type(market_data)}, Length: {len(market_data) if hasattr(market_data, '__len__') else 'N/A'}")
        ...
        if isinstance(market_data, list):
            ...
            for item in market_data:
                ...
                try:
                    ...
                    # ENHANCED: Get standardized symbol using database-aware TokenMappingManager
                    if source == 'database':
                        ...
                        # ENHANCED: Database-first symbol conversion with fallback
                        symbol = None
                        try:
                            # First: Try database lookup (most current data)
                            symbol = self.config.token_mapper.database_lookup_coingecko_id_to_symbol(chain_value)
                            if symbol:
                                logger.logger.debug(f"✅ Database lookup: {chain_value} -> {symbol}")
                            else:
                        ...
                        standardized_item = {
                            ...
                            # Preserve original database fields for backward compatibility
                            'id': item.get('id'),
                            'chain': chain_value,
                        ...
                        try:
                            ...
                            symbol = self.config.token_mapper.database_lookup_coingecko_id_to_symbol(coingecko_id)
                            if symbol:
                        ...
                        # CoinGecko format is mostly standardized already
                        ...
                        try:
                            # First: Try database lookup (most current mapping)
                            symbol = self.config.token_mapper.database_lookup_cmc_slug_to_symbol(cmc_slug)
                            if symbol:
                        ...
                        standardized_item = {
                            ...
                            'volume': self._safe_float(quote_data.get('volume_24h', 0)),
                    ...
                    # ENHANCED: Fallback handling with database validation
                    else:
                        # Handle direct symbol format or unknown format
                        symbol = item.get('symbol', 'UNKNOWN').upper()
                        if symbol and symbol != 'UNKNOWN':
                            # Try to validate symbol through database lookups
                            try:
                                token_info = self.config.token_mapper.get_token_from_all_sources(symbol, 'symbol')
            ...
            logger.logger.info(f"✅ Standardized {len(result)} tokens using enhanced database-aware TokenMappingManager in {processing_time:.3f}s")
            
            if result:
                sample_symbols = list(result.keys())[:5]
                logger.logger.debug(f"📊 Sample symbols: {sample_symbols}")
    ...
    def _detect_item_source(self, item):
        """
        Detect the data source format of a market data item using TokenMappingManager
        FIXED: Database format detection prioritized to fix 'int' object has no attribute 'upper' error
        Enhanced with comprehensive TokenMappingManager integration for 150+ token compatibility
        
        Args:
            item (dict): Market data item to analyze
            
        Returns:
            str: Source type ('database', 'coingecko', 'coinmarketcap', 'unknown')
        """
        if not isinstance(item, dict):
            return 'unknown'
        
        try:
            # Use Config's detect_data_source if available (most comprehensive)
            if hasattr(self.config, 'detect_data_source'):
                detected_source = self.config.detect_data_source(item)
                if detected_source and detected_source != 'unknown':
                    logger.logger.debug(f"✅ Source detected via Config: {detected_source}")
                    return detected_source
        except Exception as e:
            logger.logger.debug(f"⚠️ Config detection failed, using fallback: {e}")
        
        # Fallback detection with database-first priority (CRITICAL FIX)
        
        # PRIORITY 1: Database record format - CHECK FIRST to fix the int.upper() error
        # Database records have 'chain' field with string token names like 'AAVE', 'BTC'
        if 'chain' in item and 'price' in item:
            # This catches the problematic records: {'id': 5398, 'chain': 'AAVE', 'price': 283.58, ...}
            chain_value = item.get('chain')
            
            # Enhanced validation using TokenMappingManager
            if hasattr(self.config, 'token_mapper') and chain_value:
                try:
                    # Validate that this chain value is a known token
                    is_valid = False
                    
                    # Check if it's a known database name or symbol
                    if self.config.token_mapper.database_name_to_symbol(chain_value):
                        is_valid = True
                    elif self.config.token_mapper.coingecko_id_to_symbol(chain_value):
                        is_valid = True
                    elif self.config.token_mapper.is_supported_token(chain_value, 'symbol'):
                        is_valid = True
                    
                    if is_valid:
                        logger.logger.debug(f"✅ Source detected: database (validated chain='{chain_value}', price={item.get('price')})")
                        return 'database'
                    else:
                        logger.logger.debug(f"⚠️ Unknown chain value, still treating as database: {chain_value}")
                        return 'database'  # Still return database to be safe
                        
                except Exception as e:
                    logger.logger.debug(f"⚠️ Chain validation failed: {e}")
                    
            logger.logger.debug(f"✅ Source detected: database (chain='{chain_value}', price={item.get('price')})")
            return 'database'
        
        # PRIORITY 2: CoinGecko price history/chart format - has 'prices', 'market_caps', 'total_volumes'
        if 'prices' in item and 'market_caps' in item and 'total_volumes' in item:
            logger.logger.debug(f"✅ Source detected: coingecko_chart (price history data)")
            return 'coingecko'  # Treat as CoinGecko for processing
        
        # PRIORITY 3: Check for table_source field (newer database records with API source tracking)
        if 'table_source' in item:
            table_source = item['table_source']
            if table_source in ['coingecko', 'coinmarketcap']:
                logger.logger.debug(f"✅ Source detected: database (table_source='{table_source}')")
                return 'database'  # It's a database record, regardless of original API
        
        # PRIORITY 4: CoinMarketCap format - has 'slug' and nested 'quote' structure
        if 'slug' in item and ('quote' in item or any(key.startswith('quote_') for key in item.keys())):
            slug_value = item.get('slug')
            
            # Enhanced validation using TokenMappingManager
            if hasattr(self.config, 'token_mapper') and slug_value:
                try:
                    # Validate that this is a known CMC slug
                    if self.config.token_mapper.cmc_slug_to_symbol(slug_value):
                        logger.logger.debug(f"✅ Source detected: coinmarketcap (validated slug='{slug_value}')")
                        return 'coinmarketcap'
                except Exception as e:
                    logger.logger.debug(f"⚠️ CMC slug validation failed: {e}")
                    
            logger.logger.debug(f"✅ Source detected: coinmarketcap (slug='{slug_value}')")
            return 'coinmarketcap'
        
        # PRIORITY 5: CoinGecko format - has 'id' (string) and 'current_price'
        # Only check this AFTER database check to avoid catching database records with numeric 'id'
        if 'id' in item and 'current_price' in item:
            item_id = item.get('id')
            
            # Enhanced validation: distinguish between CoinGecko string IDs and database numeric IDs
            if isinstance(item_id, str):
                # Additional TokenMappingManager validation
                if hasattr(self.config, 'token_mapper'):
                    try:
                        # Validate that this is a known CoinGecko ID
                        if self.config.token_mapper.coingecko_id_to_symbol(item_id):
                            logger.logger.debug(f"✅ Source detected: coingecko (validated id='{item_id}', current_price={item.get('current_price')})")
                            return 'coingecko'
                    except Exception as e:
                        logger.logger.debug(f"⚠️ CoinGecko ID validation failed: {e}")
                        
                logger.logger.debug(f"✅ Source detected: coingecko (id='{item_id}', current_price={item.get('current_price')})")
                return 'coingecko'
            else:
                # This is likely a database record with numeric ID that we missed above
                logger.logger.debug(f"⚠️ Numeric ID detected, likely database record: {item_id}")
                return 'database'
        
        # PRIORITY 6: Additional database record checks
        if 'coin_id' in item or 'cmc_id' in item:
            coin_id = item.get('coin_id') or item.get('cmc_id')
            logger.logger.debug(f"✅ Source detected: database (coin_id/cmc_id='{coin_id}')")
            return 'database'
        
        # PRIORITY 7: Symbol-based detection with TokenMappingManager validation
        if 'symbol' in item:
            symbol_value = item.get('symbol')
            
            # Enhanced symbol validation using TokenMappingManager
            if hasattr(self.config, 'token_mapper') and symbol_value:
                try:
                    # Check if this symbol is known in our system
                    token_info = self.config.token_mapper.get_token_from_all_sources(symbol_value, 'symbol')
                    if token_info and token_info.get('found_in_sources'):
                        logger.logger.debug(f"✅ Source detected: generic API (validated symbol='{symbol_value}', sources: {token_info['found_in_sources']})")
                        return 'coingecko'  # Treat as CoinGecko-like for processing
                except Exception as e:
                    logger.logger.debug(f"⚠️ Symbol validation failed: {e}")
                    
            logger.logger.debug(f"✅ Source detected: generic API (symbol='{symbol_value}')")
            return 'coingecko'  # Treat as CoinGecko-like for processing
        
        # PRIORITY 8: Final fallback - check for any ID field and try to identify type
        if 'id' in item:
            item_id = item.get('id')
            
            # Try to identify ID type using TokenMappingManager
            if hasattr(self.config, 'token_mapper'):
                try:
                    # If it's a string, try as CoinGecko ID
                    if isinstance(item_id, str) and self.config.token_mapper.coingecko_id_to_symbol(str(item_id)):
                        logger.logger.debug(f"✅ Source detected: coingecko (fallback ID validation='{item_id}')")
                        return 'coingecko'
                    # If numeric, likely database
                    elif isinstance(item_id, (int, float)):
                        logger.logger.debug(f"✅ Source detected: database (fallback numeric ID={item_id})")
                        return 'database'
                except Exception as e:
                    logger.logger.debug(f"⚠️ Fallback ID validation failed: {e}")
        
        # Unknown format
        available_fields = list(item.keys())[:5]
        logger.logger.warning(f"⚠️ Unknown data source format, available fields: {available_fields}")
        
        # Final fallback based on field patterns
        if any(field in item for field in ['price', 'volume', 'market_cap']) and not any(field in item for field in ['current_price', 'total_volume']):
            logger.logger.debug("⚠️ Fallback: treating as database based on field patterns")
            return 'database'
    ...
    def _post_analysis(self, tweet_text: str, timeframe: str = "1h") -> bool:
        """
        Modernized post analysis method with timeout protection and proper connection handling
        ...
        start_time = time.time()
        MAX_PROCESSING_TIME = 15.0  # Maximum 15 seconds allowed for posting
        
        # Fast check for correlation reports to apply special handling
        is_correlation_report = False
        if "CORRELATION REPORT" in tweet_text or "correlation" in tweet_text.lower():
            is_correlation_report = True
            logger.logger.info("📊 IDENTIFIED AS CORRELATION REPORT - Using specialized handling")
        
        # ================================================================
        # 📝 CONTENT VALIDATION AND PREPARATION
        # ================================================================
        
        # Validate content exists and isn't empty
        if not tweet_text or tweet_text.strip() == "":
            logger.logger.error("❌ POST FAILED - Empty content")
            return False
        
        # Special fast truncation for correlation reports to avoid timeout
        if is_correlation_report and len(tweet_text) > 250:
            # Simple truncation for correlation reports - fast and effective
            tweet_text = tweet_text.split("\n\n")[0] + "\n\nCorrelation data available for multiple tokens. See full report for details."
            logger.logger.info(f"📊 Fast truncated correlation report to {len(tweet_text)} chars")
        
        # Regular content validation and replacement (simplified for performance)
        if not tweet_text or len(tweet_text.strip()) < 10:
            # Use simple fallbacks instead of complex logic
            crypto_buzz_phrases = [
                "Market update: Correlation analysis complete",
                "Crypto market analysis: Multiple tokens showing movement", 
                "Technical analysis: Market structure update"
            ]
            tweet_text = random.choice(crypto_buzz_phrases)
            logger.logger.info(f"✅ CONTENT REPLACEMENT: Used fallback phrase")
        
        # ================================================================
        # 🔗 BROWSER & DRIVER STATE VALIDATION (FAST PATH)
        # ================================================================
        ...
        # ================================================================
        # 🏠 POSTING CONTENT TO TWITTER (WITH TIMEOUT PROTECTION)
        # ================================================================
        
        try:
            # Check if we're approaching timeout already
            if time.time() - start_time > MAX_PROCESSING_TIME * 0.3:
                logger.logger.warning("⏱️ Approaching timeout in pre-posting phase - using fast path")
                
                # Fast path - navigate directly to compose without additional checks
                self.browser.driver.get('https://twitter.com/compose/tweet')
                time.sleep(1)
            else:
                # Normal path - ensure we're on Twitter home
                current_url = self.browser.driver.current_url
                if "twitter.com" not in current_url and "x.com" not in current_url:
                    self.browser.driver.get('https://twitter.com/home')
                    time.sleep(2)
            
            # Find and click text area with timeout protection
            try:
                ...
                # If compose area not found, try navigating directly to compose
                self.browser.driver.get('https://twitter.com/compose/tweet')
                time.sleep(2)
                text_area = WebDriverWait(self.browser.driver, 5).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, '[data-testid="tweetTextarea_0"]'))
            ...
            # Ensure tweet text is compatible with Twitter
            safe_tweet_text = ''.join(char for char in tweet_text if ord(char) < 0x10000)
            
            # Truncate if still too long (Twitter has a 280 char limit)
            if len(safe_tweet_text) > 275:
                safe_tweet_text = safe_tweet_text[:272] + "..."
            
            # Send tweet text
            text_area.send_keys(safe_tweet_text)
            time.sleep(1)
            
            # Check timeout again before attempting to post
            if time.time() - start_time > MAX_PROCESSING_TIME * 0.7:
                logger.logger.warning("⏱️ Approaching timeout before posting - expediting")
            ...
            for selector in post_button_selectors:
                try:
                    post_button = WebDriverWait(self.browser.driver, 2).until(
            ...
            if not post_button:
                logger.logger.error("❌ POST FAILED - Post button not found")
                return False
            ...
            # Record timeframe post if successful
            self.timeframe_last_post[timeframe] = datetime.now()
            
            # Set next scheduled post with some jitter to avoid detection
            jitter = random.uniform(0.9, 1.1)  # 10% jitter
            hours_to_add = 1 if timeframe == "1h" else 6 if timeframe == "24h" else 24
            self.next_scheduled_posts[timeframe] = (datetime.now() + timedelta(hours=hours_to_add * jitter))
            
            logger.logger.info(f"✅ {timeframe} post completed successfully")
            logger.logger.debug(f"Next {timeframe} post scheduled for {self.next_scheduled_posts[timeframe]}")
            
            total_time = time.time() - start_time
            logger.logger.info(f"⏱️ Total posting time: {total_time:.2f}s")
            ...
            error_msg = str(e)
            logger.logger.error(f"❌ POST FAILED - {error_msg}")
            
            # Check if we're timing out and add that to the error message
            if time.time() - start_time > MAX_PROCESSING_TIME * 0.9:
                logger.logger.error(f"⏱️ POST FAILED DUE TO TIMEOUT - {time.time() - start_time:.2f}s")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if hasattr(self, '_last_api_call_time'):
                    ...
                    min_interval = 5.0  # 5 seconds between calls for free tier safety
    ...
    # Add this enhanced method to your bot.py to call ALL API routes:

    def _get_comprehensive_crypto_data(self):
        """
        🚀 COMPREHENSIVE DATA PIPELINE - ALL API SOURCES FOR MAXIMUM ALPHA 🚀
        
        Combines all specialized API routes:
        - bulk_data (CoinMarketCap) ✅ Already working
        - real_time (CoinGecko) 🆕 Now added
        - individual_tokens (CoinGecko) 🆕 Now added  
        - market_overview (CoinGecko) 🆕 Now added
        - historical_data (CoinGecko) 🆕 Now added
        
        Returns comprehensive market data for optimal trading decisions
        """
        logger.logger.info("🚀 STARTING COMPREHENSIVE DATA PIPELINE")
        logger.logger.info("🎯 Collecting data from ALL specialized API routes")
        ...
        try:
            # ================================================================
            # 1️⃣ BULK DATA (CoinMarketCap) - Your existing working call
            # ================================================================
            logger.logger.info("📊 Route 1/5: Getting bulk market data (CoinMarketCap)")
            bulk_data = self._get_crypto_data()  # Your existing method
            comprehensive_data['bulk_data'] = bulk_data
            logger.logger.info(f"✅ Bulk data: {len(bulk_data) if bulk_data else 0} tokens")
            
            # ================================================================
            # 2️⃣ REAL-TIME DATA (CoinGecko) - NEW
            # ================================================================
            logger.logger.info("📊 Route 2/5: Getting real-time prices (CoinGecko)")
            try:
                # Force CoinGecko by requesting small amount with real-time flag
                real_time_params = {
                    'vs_currency': 'usd',
                    'order': 'market_cap_desc',
                    'per_page': 10,  # Small request to trigger CoinGecko
                    'page': 1,
                    'sparkline': False,  # Real-time focused
                    'price_change_percentage': '1h'  # Real-time changes
                }
                
                real_time_data = self.api_manager.get_market_data(
                    params=real_time_params,
                    timeframe='1h',  # Real-time timeframe
                    priority_tokens=None,
                    include_price_history=False
                )
                comprehensive_data['real_time'] = real_time_data
                logger.logger.info(f"✅ Real-time data: {len(real_time_data) if real_time_data else 0} tokens")
                
            except Exception as rt_error:
                logger.logger.warning(f"⚠️ Real-time data failed: {str(rt_error)}")
                comprehensive_data['real_time'] = None
            
            # ================================================================
            # 3️⃣ INDIVIDUAL TOKENS (CoinGecko) - NEW
            # ================================================================
            logger.logger.info("📊 Route 3/5: Getting individual token data (CoinGecko)")
            try:
                # Get detailed data for key tokens to trigger individual_tokens route
                key_tokens = ['bitcoin', 'ethereum', 'solana', 'ripple', 'binancecoin']
                
                individual_data = self.api_manager.get_market_data(
                    params={'ids': ','.join(key_tokens)},
                    timeframe='24h',
                    priority_tokens=key_tokens,
                    include_price_history=False
                )
                comprehensive_data['individual_tokens'] = individual_data
                logger.logger.info(f"✅ Individual tokens: {len(individual_data) if individual_data else 0} tokens")
                
            except Exception as ind_error:
                logger.logger.warning(f"⚠️ Individual tokens failed: {str(ind_error)}")
                comprehensive_data['individual_tokens'] = None
            
            # ================================================================
            # 4️⃣ MARKET OVERVIEW (CoinGecko) - NEW
            # ================================================================
            logger.logger.info("📊 Route 4/5: Getting market overview (CoinGecko)")
            try:
                # Get trending and market overview data
                if hasattr(self.api_manager, 'get_trending_tokens'):
                    trending_data = self.api_manager.get_trending_tokens()
                    comprehensive_data['market_overview'] = trending_data
                    logger.logger.info(f"✅ Market overview: {len(trending_data) if trending_data else 0} trending tokens")
                else:
                    # Fallback: small market overview request
                    overview_params = {
                        'vs_currency': 'usd',
                        'order': 'market_cap_desc', 
                        'per_page': 15,  # Medium size for overview
                        'page': 1,
                        'sparkline': True  # Overview with trends
                    }
                    
                    overview_data = self.api_manager.get_market_data(
                        params=overview_params,
                        timeframe='24h',
                        priority_tokens=None,
                        include_price_history=False
                    )
                    comprehensive_data['market_overview'] = overview_data
                    logger.logger.info(f"✅ Market overview: {len(overview_data) if overview_data else 0} tokens")
                    
            except Exception as overview_error:
                logger.logger.warning(f"⚠️ Market overview failed: {str(overview_error)}")
                comprehensive_data['market_overview'] = None
            
            # ================================================================
            # 5️⃣ HISTORICAL DATA (CoinGecko) - NEW
            # ================================================================
            logger.logger.info("📊 Route 5/5: Getting historical data (CoinGecko)")
            try:
                # Get historical data for priority tokens
                priority_tokens = ['bitcoin', 'ethereum', 'solana']
                
                historical_data = self.api_manager.get_market_data(
                    params={'ids': ','.join(priority_tokens)},
                    timeframe='7d',
                    priority_tokens=priority_tokens,
                    include_price_history=True  # This should trigger historical_data route
                )
                comprehensive_data['historical_data'] = historical_data
                logger.logger.info(f"✅ Historical data: {len(historical_data) if historical_data else 0} tokens with history")
                
            except Exception as hist_error:
                logger.logger.warning(f"⚠️ Historical data failed: {str(hist_error)}")
                comprehensive_data['historical_data'] = None
            
            # ================================================================
            # 6️⃣ COMBINE ALL DATA FOR COMPREHENSIVE OVERVIEW
            # ================================================================
            logger.logger.info("📊 Final Step: Combining all data sources")
            
            combined_data: List[Dict[str, Any]] = []
            data_sources_used: List[str] = []
            
            # Combine all successful data sources
            for source_name, source_data in comprehensive_data.items():
                ...
                if source_data and isinstance(source_data, list):
                    ...
                    logger.logger.debug(f"📊 Added {len(source_data)} entries from {source_name}")
            
            # Remove duplicates while preserving the most complete entries
            if combined_data:
                # Deduplicate by symbol, keeping most complete entries
                ...
                for entry in combined_data:
                    if isinstance(entry, dict):
                        ...
                        if symbol:
                            # Keep entry with more fields (more comprehensive)
                            if symbol not in seen_symbols or len(entry) > len(seen_symbols[symbol]):
                                seen_symbols[symbol] = entry
                
                final_data: List[Dict[str, Any]] = list(seen_symbols.values())
                comprehensive_data['combined'] = final_data
                
                logger.logger.info(f"✅ COMPREHENSIVE DATA PIPELINE COMPLETE:")
                logger.logger.info(f"📊 Sources used: {', '.join(data_sources_used)}")
                logger.logger.info(f"📊 Total unique tokens: {len(final_data)}")
                logger.logger.info(f"📊 Data completeness: {len(data_sources_used)}/5 routes successful")
                
                return final_data
            else:
                logger.logger.warning("⚠️ No data collected from any source")
                return bulk_data  # Fallback to working bulk data
            
        except Exception as e:
            logger.logger.error(f"❌ Comprehensive data pipeline failed: {str(e)}")
            # Always return the working bulk data as fallback
            return comprehensive_data.get('bulk_data') or []
    ...
    def _process_data_with_polars(self, raw_data: List[Dict[str, Any]], timeframe: str = "24h") -> Optional[Dict[str, Any]]:
        ...
        Updated to use TokenMappingManager database-aware approach instead of hardcoded mappings
        """
        try:
            start_time = time.time()
            ...
            # ================================================================
            # 🎯 TOKENMAPPINGMANAGER INTEGRATION (FULLY UPDATED - NO HARDCODED FALLBACK) 🎯
            # ================================================================

            # Use shared TokenMappingManager for database-aware token mapping
            token_mapper = None
            if self.token_mapper:
                logger.logger.debug("✅ Using shared TokenMappingManager for Polars processing")
                token_mapper = self.token_mapper
            else:
                logger.logger.warning("⚠️ Shared TokenMappingManager not available")
                logger.logger.warning("⚠️ Falling back to basic symbol extraction")

            # Add standard symbol column using TokenMappingManager database-aware approach
            if token_mapper:
                # Use TokenMappingManager for database-driven symbol standardization
                def map_token_symbol(token_id):
                    try:
                        # PRIMARY: Try database lookup first
                        symbol = token_mapper.database_lookup_coingecko_id_to_symbol(token_id)
                        if symbol:
                            logger.logger.debug(f"✅ Database lookup: {token_id} -> {symbol}")
                            return symbol
                        
                        # SECONDARY: Try get_token_from_all_sources as comprehensive fallback
                        token_info = token_mapper.get_token_from_all_sources(token_id, 'coingecko_id')
                        if token_info and 'symbol' in token_info and token_info['symbol']:
                            symbol = token_info['symbol'].upper()
                            logger.logger.debug(f"✅ All sources lookup: {token_id} -> {symbol}")
                            return symbol
                        
                        # TERTIARY: Try reverse lookup through hardcoded mappings (no hardcoded list)
                        symbol = token_mapper.coingecko_id_to_symbol(token_id)
                        if symbol and symbol != token_id.upper():  # Valid mapping found
                            logger.logger.debug(f"✅ Hardcoded mapping lookup: {token_id} -> {symbol}")
                            return symbol
                        
                        # LAST RESORT: Use token_id as symbol (capitalized)
                        # This ensures we never fail completely but don't maintain hardcoded lists
                        fallback_symbol = str(token_id).upper()
                        logger.logger.debug(f"⚠️ LAST RESORT: {token_id} -> {fallback_symbol} (no mapping found)")
                        return fallback_symbol
                        
                    except Exception as mapping_error:
                        # Even in error case, return capitalized token_id rather than maintain hardcoded list
                        fallback_symbol = str(token_id).upper()
                        logger.logger.debug(f"❌ MAPPING ERROR: {token_id} -> {fallback_symbol} (error: {str(mapping_error)})")
                        return fallback_symbol
                
                df = df.with_columns([
                    pl.col('id').map_elements(map_token_symbol, return_dtype=pl.Utf8).alias('standard_symbol')
                ])
                logger.logger.debug("✅ Applied TokenMappingManager-based symbol standardization (database-first, no hardcoded fallback)")
            else:
                # Basic fallback if TokenMappingManager unavailable - use existing symbol from API
                df = df.with_columns([
                    pl.col('symbol').str.to_uppercase().alias('standard_symbol')
                ])
                logger.logger.warning("⚠️ Using basic symbol standardization fallback (API symbol only)")
            ...
            # ================================================================
            # 🔧 VECTORIZED DATA TRANSFORMATION (ENHANCED) 🔧
            # ================================================================
            ...
            for row in df.iter_rows(named=True):
                try:
                    ...
                    base_token_data = {
                        ...
                        'total_volume': float(row['total_volume']),  # Keep both for compatibility
                    ...
                    if 'price_change_percentage_1h_in_currency' in row:
                        base_token_data['price_change_percentage_1h_in_currency'] = float(row['price_change_percentage_1h_in_currency'])
                    if 'price_change_percentage_7d_in_currency' in row:
                        base_token_data['price_change_percentage_7d_in_currency'] = float(row['price_change_percentage_7d_in_currency'])
                
                    # ================================================================
                    # 🎯 SEPARATE OBJECTS FOR BETTER TRACKING (FIXED) 🎯
                    # ================================================================
                    
                    # Create separate objects instead of duplicate references
                    # This prevents database conflicts while maintaining full tracking capability
                    
                    # Symbol-based entry (primary)
                    symbol_data = base_token_data.copy()
                    symbol_data.update({
                        '_lookup_type': 'symbol',
                        '_primary_key': symbol,
                        '_secondary_key': token_id,
                        '_token_source': 'symbol_lookup'
                    })
                    result[symbol] = symbol_data
                    
                    # ID-based entry (secondary) - separate object with additional metadata
                    id_data = base_token_data.copy()
                    id_data.update({
                        '_lookup_type': 'token_id',
                        '_primary_key': token_id,
                        '_secondary_key': symbol,
                        '_token_source': 'id_lookup'
                    })
                    result[token_id] = id_data
                    
                    successful_tokens.append(symbol)
                    logger.logger.debug(f"✅ Processed {symbol} ({token_id}) with separate tracking objects")
            ...
            # ================================================================
            # 💾 DATABASE STORAGE WITH CONNECTION MANAGEMENT 💾
            # ================================================================
            
            # Store all data with proper database connection management
            if result:
                try:
                    # Check database connection before storage
                    if not hasattr(self, 'db') or self.db is None:
                        logger.logger.warning("⚠️ Database not initialized for batch storage")
                        try:
                            from database import CryptoDatabase
                            self.db = CryptoDatabase()
                            logger.logger.info("✅ Created database connection for Polars batch storage")
                        except Exception as db_init_error:
                            logger.logger.error(f"❌ Database initialization failed: {str(db_init_error)}")
                            # Continue without storage
                    
                    # Perform batch storage with connection verification
                    if hasattr(self, 'db') and self.db:
                        self._store_market_data_batch(result, successful_tokens)
                        logger.logger.debug(f"✅ Batch storage completed for {len(successful_tokens)} tokens")
                    else:
                        logger.logger.warning("⚠️ Skipping database storage - no connection available")
            ...
            logger.logger.info(f"📊 Total result entries: {len(result)} (symbol + ID entries)")
            ...
            if result:
                ...
                processing_metadata = {
                    ...
                    '_token_mapper_used': token_mapper is not None,
                    '_separate_objects': True  # Flag indicating we use separate objects instead of references
                }
    ...
    def _get_historical_price_data(self, token: str, hours: int, timeframe: Optional[str] = None) -> Union[List[Dict[str, Any]], str]:
        ...
        try:
            ...
            if not results and self.token_mapper:
                logger.logger.debug(f"No results for {token}, trying shared TokenMappingManager")
                try:
                    coingecko_id = self.token_mapper.symbol_to_coingecko_id(token)
                    if coingecko_id:
                        ...
                        results = cursor.fetchall()
                        if results:
                            logger.logger.debug(f"✅ Found {len(results)} results using TokenMappingManager mapping")
        ...
        try:
            ...
            for timeframe, predictions in expired_by_timeframe.items():
                for prediction in predictions:
                    ...
                    # ATTEMPT 1.5: Use TokenMappingManager database_name_to_symbol conversion
                    if not token_data and hasattr(self, 'config') and hasattr(self.config, 'token_mapper'):
                        try:
                            standardized_symbol = self.config.token_mapper.database_name_to_symbol(token)
                            if standardized_symbol != token and standardized_symbol in market_data:
                                token_data = market_data[standardized_symbol]
                                lookup_attempts.append(f"✅ TokenMappingManager DB conversion: '{token}' → '{standardized_symbol}' found")
                            else:
                                lookup_attempts.append(f"❌ TokenMappingManager DB conversion: '{token}' → '{standardized_symbol}' not found")
                        except Exception as token_mapper_error:
                            lookup_attempts.append(f"❌ TokenMappingManager DB conversion error: {str(token_mapper_error)}")    
    ...
    def _run_analysis_cycle(self) -> None:
        ...
        try:
            ...
            raw_market_data = self._get_comprehensive_crypto_data()
            ...
            # Enhanced intersection check using TokenMappingManager
            common_tokens = []

            # Use shared TokenMappingManager if available
            if self.token_mapper:
                logger.logger.debug("✅ Using shared TokenMappingManager for intersection checking")
            else:
                logger.logger.debug("⚠️ Shared TokenMappingManager not available for intersection checking")

            # Smart intersection check with multiple format attempts
            for ref_token in self.reference_tokens:
                # Direct match first
                if ref_token in market_data:
                    common_tokens.append(ref_token)
                    continue
                
                # Try TokenMappingManager conversions if available
                if self.token_mapper:
                    # Try converting symbol to CoinGecko ID (BTC -> bitcoin)
                    try:
                        coingecko_id = self.token_mapper.symbol_to_coingecko_id(ref_token)
                        if coingecko_id and coingecko_id in market_data:
                            common_tokens.append(ref_token)
                            logger.logger.debug(f"✅ TokenMappingManager: {ref_token} -> {coingecko_id} found in market_data")
                            continue
                    except Exception:
                        pass
                    
                    # Try converting symbol to database name (BTC -> BITCOIN)
                    try:
                        # Get database name from token info (correct method)
                        token_info = self.token_mapper.get_token_info(ref_token)
                        database_name = token_info.get('database_name') if token_info else None
                            
                        if database_name and database_name in market_data:
                            common_tokens.append(ref_token)
                            logger.logger.debug(f"✅ TokenMappingManager: {ref_token} -> {database_name} found in market_data")
                            continue
                    except Exception:
                        pass
                ...
                for market_key in market_data.keys():
                    if ref_token_upper == market_key.upper():
                        common_tokens.append(ref_token)
                        logger.logger.debug(f"✅ Case-insensitive match: {ref_token} -> {market_key}")
                        break

            # =====================================================================

            # Enhanced intersection analysis with TokenMappingManager
            ...
            if common_tokens:
                ...
                # Enhanced debugging with TokenMappingManager
                logger.logger.debug("🔍 DEBUGGING TOKEN FORMAT MISMATCH:")
                logger.logger.debug(f"📊 Reference tokens sample: {self.reference_tokens[:5]}")
                logger.logger.debug(f"📊 Market data keys sample: {list(market_data.keys())[:5]}")
                ...
                # Try TokenMappingManager debugging if available
                if self.token_mapper:
                    logger.logger.debug("🔍 TokenMappingManager conversion attempts:")
                    debug_conversions = []
                    for ref_token in self.reference_tokens[:3]:  # Sample first 3 tokens
                        try:
                            coingecko_id = self.token_mapper.symbol_to_coingecko_id(ref_token)
                            token_info = self.token_mapper.get_token_info(ref_token)
                            database_name = token_info.get('database_name') if token_info else 'N/A'
                            
                            debug_conversions.append(f"{ref_token} -> CoinGecko: {coingecko_id}, Database: {database_name}")
                        except Exception as conv_error:
                            debug_conversions.append(f"{ref_token} -> Error: {str(conv_error)}")
                    
                    for conversion in debug_conversions:
                        logger.logger.debug(f"   🔄 {conversion}")
                
                # Suggest possible fixes
                logger.logger.warning("💡 Possible fixes:")
                logger.logger.warning("   1. Ensure market_data standardization is working correctly")
                logger.logger.warning("   2. Check TokenMappingManager database table coverage")
                logger.logger.warning("   3. Verify reference_tokens are being generated from correct database tables")
    ...
    def _has_required_market_data(self, token_data):
        """
        Check if a token has the required market data for correlation analysis
        
        Args:
            token_data: Dictionary containing token market data
            
        Returns:
            Boolean indicating if token has sufficient data
        """
        # Check for essential fields needed for correlation
        required_fields = [
            'price_change_percentage_24h',
            'market_cap'
        ]
        
        # Verify all required fields exist and are not None
        ...
        try:
            # Ensure price change is a valid number
            price_change = float(token_data['price_change_percentage_24h'])
            ...
            # Price change can be any value (positive or negative)
            # but should be a valid number (not NaN)
            if math.isnan(price_change):
                return False
                
            return True
        except (ValueError, TypeError):
            # If conversion to float fails, data is invalid
            return False
    ...
    def _generate_correlation_report(self, market_data: Dict[str, Any], timeframe: str = "24h") -> str:
        """
        Generate correlation report with proper database connection handling and timeout protection
        Updated to use TokenMappingManager database-aware approach instead of direct _get_connection()
        ...
        Returns:
            Formatted correlation report text or empty string if generation fails/times out
        """
        # =========================================
        # INITIALIZATION AND VALIDATION
        # =========================================
        function_start = time.time()
        MAX_EXECUTION_TIME = 3.0  # Maximum 3 seconds
        ...
        try:
            # Validate timeframe
            if timeframe not in ["1h", "24h", "7d"]:
                logger.logger.warning(f"Invalid timeframe {timeframe}, defaulting to 24h")
                timeframe = "24h"
            
            logger.logger.info(f"📊 IDENTIFIED AS CORRELATION REPORT - Using specialized handling")
            
            # Check if correlation matrix is globally disabled
            if getattr(self, 'disable_correlation_matrix', True):
                logger.logger.debug(f"🚫 Correlation globally disabled for {timeframe}")
                return ""
            
            # Check if we have enough market data
            if not market_data or len(market_data) < 2:
                logger.logger.warning(f"Insufficient market data for correlation analysis")
                return ""
            
            # Quick timeout check
            if time.time() - function_start > MAX_EXECUTION_TIME * 0.1:
                logger.logger.warning("⏱️ Early timeout detected - aborting correlation analysis")
                return ""
            
            # =========================================
            # DATABASE CONNECTION SETUP (FIXED)
            # =========================================
            # Use database-aware approach instead of direct _get_connection()
            if not hasattr(self, 'db') or self.db is None:
                logger.logger.error("❌ Database not initialized for correlation report")
                try:
                    from database import CryptoDatabase
                    self.db = CryptoDatabase()
                    logger.logger.info("✅ Created new database connection for correlation report")
                except Exception as db_init_error:
                    logger.logger.warning(f"⚠️ Database initialization failed: {str(db_init_error)}")
                    # Continue without database storage
            
            # =========================================
            # TOKEN SELECTION AND MAPPING
            # =========================================
            # Use TokenMappingManager for token selection if available
            tokens_to_analyze = []
            
            # Try to get tokens using your updated database-aware approach
            try:
                # Use the database-driven token selection method you created
                database_tokens = self.get_tokens_with_recent_data_by_market_cap(hours=24, limit=15)
                if database_tokens:
                    tokens_to_analyze = database_tokens[:10]  # Limit to top 10 for correlation analysis
                    logger.logger.debug(f"✅ Using database-driven tokens: {tokens_to_analyze}")
                else:
                    # Fallback to market_data keys
                    tokens_to_analyze = list(market_data.keys())[:10]
                    logger.logger.warning(f"⚠️ FALLBACK: Using market_data keys for tokens")
            except Exception as token_selection_error:
                logger.logger.warning(f"❌ FALLBACK: Token selection failed: {str(token_selection_error)}")
                tokens_to_analyze = list(market_data.keys())[:10]
            
            # Additional timeout check
            if time.time() - function_start > MAX_EXECUTION_TIME * 0.3:
                logger.logger.warning("⏱️ Timeout during token selection - continuing with reduced analysis")
                tokens_to_analyze = tokens_to_analyze[:5]  # Reduce scope
            ...
            for token in tokens_to_analyze:
                ...
                if time.time() - function_start > MAX_EXECUTION_TIME * 0.6:
                    logger.logger.warning("⏱️ Timeout during correlation calculation - truncating analysis")
                    break
                
                # Get token data using multiple approaches
                token_data = None
                
                # 1. Direct lookup
                token_data = market_data.get(token)
                
                # 2. TokenMappingManager lookup if available
                if not token_data and hasattr(self, 'config') and hasattr(self.config, 'token_mapper'):
                    try:
                        # Try symbol to coingecko ID conversion
                        coingecko_id = self.config.token_mapper.symbol_to_coingecko_id(token)
                        if coingecko_id and coingecko_id in market_data:
                            token_data = market_data[coingecko_id]
                            logger.logger.debug(f"✅ TokenMappingManager: {token} -> {coingecko_id}")
                    except Exception as mapping_error:
                        logger.logger.debug(f"TokenMappingManager lookup failed for {token}: {str(mapping_error)}")
                
                # 3. Database lookup fallback
                if not token_data and hasattr(self, 'db') and self.db:
                    try:
                        # Use database-aware connection approach
                        conn, cursor = self.db._get_connection()
                        cursor.execute("""
                            SELECT coin_id FROM coingecko_market_data 
                            WHERE UPPER(symbol) = ? OR UPPER(coin_id) = ?
                            ORDER BY timestamp DESC
                            LIMIT 1
                        """, (token.upper(), token.upper()))
                        
                        result = cursor.fetchone()
                        if result and result['coin_id'] in market_data:
                            token_data = market_data[result['coin_id']]
                            logger.logger.debug(f"✅ Database lookup: {token} -> {result['coin_id']}")
                ...
                if token_data and isinstance(token_data, dict):
                    try:
                        ...
                        if current_price > 0 and market_cap > 0:
                            ...
                            market_metrics['total_volume'] += float(volume_24h) if volume_24h else 0
                            market_metrics['avg_24h_change'] += float(price_change_24h) if price_change_24h else 0
                            market_metrics['tokens_analyzed'] += 1
                            
                            if float(price_change_24h) > 0:
                                market_metrics['bullish_tokens'] += 1
                            elif float(price_change_24h) < 0:
                                market_metrics['bearish_tokens'] += 1
                                
                    except Exception as processing_error:
                        logger.logger.debug(f"Error processing {token}: {str(processing_error)}")
            
            # Calculate averages
            if market_metrics['tokens_analyzed'] > 0:
                market_metrics['avg_24h_change'] /= market_metrics['tokens_analyzed']
            
            # Build correlation matrix for valid tokens
            for i, token_a in enumerate(valid_tokens):
                if time.time() - function_start > MAX_EXECUTION_TIME * 0.7:
                    break
                    
                matrix[token_a] = {}
                
                for j, token_b in enumerate(valid_tokens):
                    if i == j:
                        matrix[token_a][token_b] = 1.0  # Perfect self-correlation
                    else:
                        try:
                            # Simple correlation based on price changes
                            data_a = market_data.get(token_a, {})
                            data_b = market_data.get(token_b, {})
                            
                            change_a = float(data_a.get('price_change_percentage_24h', 0))
                            change_b = float(data_b.get('price_change_percentage_24h', 0))
                            
                            # Simple correlation heuristic
                            if abs(change_a) < 0.1 and abs(change_b) < 0.1:
                                correlation = 0.5  # Both stable
                            elif change_a * change_b > 0:
                                correlation = min(0.8, 0.3 + abs(change_a * change_b) / 100)  # Same direction
                            else:
                                correlation = max(-0.8, -0.3 - abs(change_a * change_b) / 100)  # Opposite direction
                            
                            matrix[token_a][token_b] = correlation
                            total_correlation_value += abs(correlation)
                            total_correlations += 1
                            
                        except Exception as corr_error:
                            matrix[token_a][token_b] = 0.0
            
            # =========================================
            # REPORT GENERATION
            # =========================================
            fallback_report = f"📊 BRIEF MARKET UPDATE ({timeframe.upper()}) - {market_metrics['tokens_analyzed']} tokens analyzed"
            
            # Check for timeout again before formatting
            if time.time() - function_start > MAX_EXECUTION_TIME * 0.7:
                logger.logger.warning("⏱️ Correlation analysis timeout during formatting - returning simple report")
            ...
            try:
                ...
                avg_correlation = total_correlation_value / total_correlations if total_correlations > 0 else 0.0
                ...
                # Only add top correlations if time permits
                if time.time() - function_start <= MAX_EXECUTION_TIME * 0.8:
                    # Extract correlations into a flat list
                    correlations_flat = []
                    for token in matrix:
                        for other_token in matrix[token]:
                            if token != other_token:
                                correlations_flat.append((token, other_token, matrix[token][other_token]))
                    ...
                    for i, (token_a, token_b, corr_value) in enumerate(correlations_flat[:3]):
                        if corr_value > 0:
                            report_lines.append(f"• {token_a.upper()}-{token_b.upper()}: +{corr_value:.2f}")
                        else:
                            report_lines.append(f"• {token_a.upper()}-{token_b.upper()}: {corr_value:.2f}")
                ...
                report_lines.extend([
                    "",
                    f"📈 Market Summary: {market_metrics['bullish_tokens']}↗ {market_metrics['bearish_tokens']}↘",
                    f"💰 Total Volume: ${market_metrics['total_volume']/1000000:.0f}M"
                ])
                
                report_text = "\n".join(report_lines)
                
                # Truncate if too long (for social media posting)
                if len(report_text) > 280:
                    truncated_lines = report_lines[:6]  # Keep header and key info
                    truncated_lines.append("...")
                    report_text = "\n".join(truncated_lines)
                    logger.logger.info(f"📊 Fast truncated correlation report to {len(report_text)} chars")
                ...
                if hasattr(self, 'db') and self.db:
                    try:
                        # Use database-aware storage approach
                        conn, cursor = self.db._get_connection()
                        
                        # Store minimal correlation data
                        cursor.execute("""
                            INSERT OR IGNORE INTO posted_content 
                            (timestamp, content, trigger_type, timeframe, sentiment)
                            VALUES (?, ?, ?, ?, ?)
                        """, (
                            datetime.now(),
                            f"CORRELATION REPORT ({timeframe.upper()})",
                            "correlation_analysis",
                            timeframe,
                            "neutral"  # Adding a default sentiment value
                        ))
                        
                        # Commit changes
                        conn.commit()
                        logger.logger.debug(f"✅ Stored minimal correlation data")
                        
                    except Exception as db_err:
                        logger.logger.warning(f"Non-critical database error: {str(db_err)}")
                        # Continue anyway since storing data is not critical
                    finally:
                ...
                logger.logger.error(f"Error formatting correlation report: {str(format_err)}")
                return fallback_report
            ...
            function_time = time.time() - function_start
            logger.logger.error(f"❌ Correlation report failed after {function_time:.3f}s")
            ...
            # Return empty string to avoid breaking the posting flow
            return ""
    ...
    def _format_prediction_tweet(self, token: str, prediction: Dict[str, Any], market_data: Dict[str, Any], timeframe: str = "1h") -> str:
        ...
        try:
            ...
            if is_trading_format:
                ...
                # Trading format conversion logic would go here (preserved from original)
                
            # ================================================================
            # 🛡️ ENHANCED FIELD VALIDATION WITH SAFE DEFAULTS
            # ================================================================
            
            logger.logger.debug(f"🔍 STEP 2: Enhanced field validation")
            
            lower_bound = 0.0
            upper_bound = 0.0

            try:
                # Extract core prediction fields with safe defaults
                price = float(pred_data.get('price', 0) or pred_data.get('predicted_price', 0))
                confidence = float(pred_data.get('confidence', 50))
                lower_bound = float(pred_data.get('lower_bound', price * 0.98))
                upper_bound = float(pred_data.get('upper_bound', price * 1.02))
                percent_change = float(pred_data.get('percent_change', 0))
                
                # ENHANCED: Ensure required fields exist with intelligent defaults
                sentiment = pred_data.get('sentiment', 'NEUTRAL')
                if sentiment not in ['BULLISH', 'BEARISH', 'NEUTRAL']:
                    # Derive sentiment from percent_change if invalid
                    if percent_change > 2:
                        sentiment = 'BULLISH'
                ...
                # ENHANCED: Generate rationale if missing or generic
                rationale = pred_data.get('rationale', '')
                
                # Define timeframe-specific terms
                timeframe_terms = {
                    '1h': 'short-term',
                    '24h': 'daily', 
                    '7d': 'weekly'
                }
                timeframe_term = timeframe_terms.get(timeframe, timeframe)
                
                if not rationale or rationale in [
                    f"Technical analysis for {token} over {timeframe} timeframe suggests potential movement.",
                    "Random Forest prediction based on limited feature data",
                    "Random Forest prediction failed, using fallback values"
                ]:
                    # Generate intelligent rationale based on available data and timeframe
                    if confidence > 70:
                        confidence_desc = "high-confidence"
                    elif confidence > 50:
                        confidence_desc = "moderate-confidence"
                    else:
                        confidence_desc = "cautious"
                    
                    direction = "upward" if percent_change > 0 else "downward" if percent_change < 0 else "sideways"
                    rationale = f"{confidence_desc} {timeframe_term} {direction} prediction based on technical analysis"
                
                # ENHANCED: Generate key_factors if missing
                key_factors = pred_data.get('key_factors', [])
                if not key_factors or key_factors in [['Insufficient data', 'Default prediction'], ['Prediction error', 'Default fallback']]:
                    key_factors = []
                    if confidence > 70:
                        key_factors.append("Strong model confidence")
                    if abs(percent_change) > 2:
                        key_factors.append("Significant price movement expected")
                    if sentiment != 'NEUTRAL':
                        key_factors.append(f"{sentiment.lower()} market signals")
                    
                    # Add timeframe-specific factors
                    timeframe_factors = {
                        '1h': "Short-term price action",
                        '24h': "Daily market dynamics",
                        '7d': "Weekly trend patterns"
                    }
                    if timeframe in timeframe_factors:
                        key_factors.append(timeframe_factors[timeframe])
                    
                    if not key_factors:
                        key_factors = ["Technical analysis", "Market conditions"]
                
                logger.logger.debug(f"✅ VALIDATED PREDICTION DATA:")
                logger.logger.debug(f"   • Price: ${price:.4f}")
                logger.logger.debug(f"   • Confidence: {confidence}%")
                logger.logger.debug(f"   • Percent change: {percent_change:+.2f}%")
                logger.logger.debug(f"   • Sentiment: {sentiment}")
                logger.logger.debug(f"   • Rationale: '{rationale[:50]}...'")
                logger.logger.debug(f"   • Key factors: {len(key_factors)} items")
                
            except (ValueError, TypeError) as e:
                logger.logger.error(f"❌ CRITICAL: Invalid prediction data types: {str(e)}")
                # Use market data as fallback for price if available
                token_data = market_data.get(token, {})
                current_price = token_data.get('current_price', 0)
                if current_price > 0:
                    price = current_price * 1.001  # Minimal prediction
                    confidence = 45
                    percent_change = 0.1
                    sentiment = 'NEUTRAL'
                    rationale = f"Conservative {timeframe} outlook for {token}"
                    key_factors = ["Market analysis", "Conservative approach"]
                else:
                    raise ValueError(f"No valid price data available for {token}")
            
            # ================================================================
            # 📊 MARKET CONTEXT AND TIMEFRAME PROCESSING
            # ================================================================
            
            logger.logger.debug(f"🔍 STEP 3: Market context processing")
            
            # Get market context
            token_data = market_data.get(token, {})
            current_price = token_data.get('current_price', price)
            price_change_24h = token_data.get('price_change_percentage_24h', 0)
            
            # Determine time context for display
            time_context_map = {
                "1h": "next hour",
                "24h": "next 24 hours", 
                "7d": "next week"
            }
            time_context = time_context_map.get(timeframe, f"next {timeframe}")
            display_timeframe = timeframe.upper()
            
            # Timeframe-specific styling preferences
            timeframe_style_preferences = {
                '1h': {'min_confidence_for_insider': 75, 'prefers_professional': False},
                '24h': {'min_confidence_for_insider': 70, 'prefers_professional': True}, 
                '7d': {'min_confidence_for_insider': 65, 'prefers_professional': True}
            }
            style_prefs = timeframe_style_preferences.get(timeframe, {'min_confidence_for_insider': 70, 'prefers_professional': False})
            
            logger.logger.debug(f"📊 MARKET CONTEXT:")
            logger.logger.debug(f"   • Current price: ${current_price:.4f}")
            logger.logger.debug(f"   • 24h change: {price_change_24h:+.2f}%")
            logger.logger.debug(f"   • Time context: {time_context}")
            
            # ================================================================
            # 🎨 DETERMINE STYLE AND FORMAT INITIAL TWEET
            # ================================================================
            
            logger.logger.debug(f"🔍 STEP 4: Determining tweet style and formatting")
            
            # Determine style based on confidence, market conditions, and timeframe preferences
            use_insider_style = self._should_use_insider_style(confidence, sentiment, rationale)
            
            # Apply timeframe-specific style adjustments
            if confidence < style_prefs['min_confidence_for_insider']:
                use_insider_style = False
            elif style_prefs['prefers_professional'] and confidence < 80:
                use_insider_style = False
            ...
            logger.logger.debug(f"🔍 STEP 5: Content length validation and expansion")
            
            # Get tweet constraints with timeframe-specific adjustments
            min_length = config.TWEET_CONSTRAINTS['MIN_LENGTH']
            max_length = config.TWEET_CONSTRAINTS['MAX_LENGTH']
            hard_stop = config.TWEET_CONSTRAINTS['HARD_STOP_LENGTH']
            
            # Timeframe-specific length preferences (longer timeframes can have more detailed content)
            timeframe_length_adjustments = {
                '1h': {'min_adjustment': 0, 'max_adjustment': 0},     # Short and punchy for 1h
                '24h': {'min_adjustment': 10, 'max_adjustment': 20},  # Slightly more detailed for daily
                '7d': {'min_adjustment': 20, 'max_adjustment': 30}    # Most detailed for weekly
            }
            length_adj = timeframe_length_adjustments.get(timeframe, {'min_adjustment': 0, 'max_adjustment': 0})
            adjusted_min_length = min_length + length_adj['min_adjustment']
            ...
            # Content expansion if needed
            final_tweet = initial_tweet
            if len(initial_tweet) < min_length:
                logger.logger.debug(f"🔧 EXPANDING: Tweet too short ({len(initial_tweet)} < {min_length})")
                expanded_content = self._expand_prediction_content(
                initial_tweet, token, prediction, market_data, timeframe, adjusted_min_length, max_length
                )
                if len(expanded_content) >= min_length:
                    final_tweet = expanded_content
                    logger.logger.debug(f"✅ EXPANSION SUCCESS: {len(initial_tweet)} → {len(final_tweet)} chars")
                else:
                    logger.logger.warning(f"⚠️ EXPANSION INSUFFICIENT: Still {len(expanded_content)} chars")
            
            # ================================================================
            # ✅ FINAL VALIDATION AND RETURN
            # ================================================================
            
            logger.logger.debug(f"🔍 STEP 6: Final validation")
            ...
            # ================================================================
            # ❌ ENHANCED EMERGENCY CONTENT GENERATION
            # ================================================================
            ...
            # ================================================================
            # 🚨 ENHANCED EMERGENCY CONTENT GENERATION WITH CLAUDE
            # ================================================================
            
            logger.logger.warning(f"🚨 GENERATING ENHANCED EMERGENCY CONTENT for {token}")
            
            try:
                # Extract whatever data we can from prediction
                emergency_price = (
                    prediction.get('price', 0) or 
                    prediction.get('predicted_price', 0) or
                ...
                emergency_sentiment = prediction.get('sentiment', 'NEUTRAL')
                
                # Try to use Claude to generate natural content instead of dry emergency format
                if hasattr(self, 'llm_provider') and self.llm_provider:
                    try:
                        # Timeframe-specific emergency prompts
                        timeframe_prompts = {
                            '1h': f"Generate a natural, conversational short-term crypto prediction tweet for {token}.",
                            '24h': f"Generate a natural, conversational daily crypto outlook tweet for {token}.", 
                            '7d': f"Generate a natural, conversational weekly crypto analysis tweet for {token}."
                        }
                        base_prompt = timeframe_prompts.get(timeframe, f"Generate a natural, conversational crypto prediction tweet for {token} ({timeframe}).")
                        
                        emergency_prompt = f"""{base_prompt}
                        
    Available data:
    - Price prediction: ${emergency_price:.4f}
    - Confidence: {emergency_confidence}%
    - Sentiment: {emergency_sentiment}
    - Timeframe: {timeframe}

    Create an engaging tweet that sounds like a knowledgeable crypto trader, not a bot. Keep it under 240 characters and avoid dry technical language. Make it conversational and interesting."""

                        enhanced_emergency = self.llm_provider.generate_text(
                            prompt=emergency_prompt,
                            max_tokens=100
                        )
                        
                        if enhanced_emergency and len(enhanced_emergency.strip()) > 20:
                            logger.logger.warning(f"✅ CLAUDE-ENHANCED EMERGENCY CONTENT GENERATED:")
                            logger.logger.debug(f"   • Length: {len(enhanced_emergency)} chars")
                            logger.logger.debug(f"   • Content: '{enhanced_emergency}'")
                            return enhanced_emergency.strip()
                    
                    except Exception as claude_error:
                        logger.logger.error(f"💥 CLAUDE EMERGENCY GENERATION FAILED: {str(claude_error)}")
                
                # Fallback: Create timeframe-specific emergency tweets
                direction = "📈" if emergency_confidence > 60 else "📊"
                confidence_phrase = "strong signals" if emergency_confidence > 70 else "moderate confidence" if emergency_confidence > 50 else "cautious outlook"

                timeframe_emergency_templates = {
                    '1h': f"📊 {token} short-term signals: ${emergency_price:.4f} target with {confidence_phrase}. Quick market scan ongoing.",
                    '24h': f"📈 {token} daily outlook: ${emergency_price:.4f} projection with {confidence_phrase}. Market analysis in progress.",
                    '7d': f"📊 {token} weekly perspective: ${emergency_price:.4f} target with {confidence_phrase}. Trend analysis developing."
                }
                ...
                if emergency_price > 0:
                    ...
                    emergency_tweet = timeframe_emergency_templates.get(
                        timeframe, 
                        f"{direction} {token} {timeframe} outlook: ${emergency_price:.4f} target with {confidence_phrase}. Market analysis ongoing."
                    )
                else:
                    timeframe_fallback_templates = {
                        '1h': f"📊 {token} short-term analysis in progress. Quick market review for optimal timing.",
                        '24h': f"📊 {token} daily analysis in progress. Market conditions under review for entry opportunities.",
                        '7d': f"📊 {token} weekly analysis developing. Trend assessment ongoing for strategic positioning."
                    }
                    emergency_tweet = timeframe_fallback_templates.get(
                        timeframe,
                        f"📊 {token} {timeframe} analysis in progress. Market conditions under review for optimal entry opportunities."
                    )
                
                logger.logger.warning(f"✅ IMPROVED EMERGENCY CONTENT GENERATED:")
                ...
                # Absolute last resort - timeframe-aware final fallback
                timeframe_final_fallbacks = {
                    '1h': f"📊 {token} short-term market scan ongoing. Updates coming soon.",
                    '24h': f"📊 {token} daily market analysis ongoing. Updates coming soon.", 
                    '7d': f"📊 {token} weekly market analysis ongoing. Updates coming soon."
                }
                final_emergency = timeframe_final_fallbacks.get(
                    timeframe, 
                    f"📊 {token} {timeframe} market analysis ongoing. Updates coming soon."
                )
                logger.logger.error(f"🆘 USING FINAL FALLBACK: '{final_emergency}'")
    ...
    def _analyze_market_sentiment(self, token, market_data, trigger_type=None, timeframe="1h"):
        ...
        compatibility with _get_vs_market_analysis, trading format support,
        and enhanced technical signal processing
        ...
        try:
            ...
            # ================================================================
            # 🚀 NEW: GET TECHNICAL ANALYSIS FOR ENHANCEMENT
            # ================================================================
            
            logger.logger.debug(f"🔍 Getting technical analysis for {token} enhancement")
            
            # Get technical analysis data - try multiple sources
            technical_analysis = None
        
            # Method 1: Try to use technical indicators directly (skip prediction engine complexity)
            try:
                from technical_indicators import analyze_technical_indicators
                
                # Get price history for technical analysis
                price_history = self._get_price_history_for_analysis(token, timeframe)
                if price_history and len(price_history) >= 10:
                    # Use technical_indicators directly - it handles missing volume/highs/lows
                    technical_analysis = analyze_technical_indicators(
                        prices=price_history,
                        highs=None,  # Let it derive from prices
                        lows=None,   # Let it derive from prices  
                        volumes=None,  # Let it use defaults
                        timeframe=timeframe
                    )
                    if technical_analysis:
                        logger.logger.debug(f"✅ Got technical analysis from technical_indicators")
            except Exception as tech_error:
                logger.logger.debug(f"Technical indicators analysis failed: {tech_error}")
            
            # Method 2: Try to get from technical indicators directly
            if not technical_analysis:
                try:
                    ...
                    if price_history and len(price_history) >= 10:
                        ...
                        logger.logger.debug(f"✅ Got technical analysis from technical_indicators")
                except Exception as direct_tech_error:
                    logger.logger.debug(f"Direct technical analysis failed: {direct_tech_error}")
            
            # Method 3: Extract from existing prediction if available
            if not technical_analysis and token in normalized_market_data:
                token_data = normalized_market_data[token]
                if isinstance(token_data, dict) and 'technical_analysis' in token_data:
                    technical_analysis = token_data['technical_analysis']
                    logger.logger.debug(f"✅ Got technical analysis from market data")

            # ================================================================
            # 🎯 ENHANCE TECHNICAL SIGNALS (NEW CODE)
            # ================================================================
            
            enhanced_sentiment = None
            
            if technical_analysis and isinstance(technical_analysis, dict):
                logger.logger.debug(f"🚀 Enhancing technical signals for {token}")
                
                # Call the enhancement method
                enhanced_technical = self._enhance_technical_signals(technical_analysis, token, timeframe)
                
                if enhanced_technical and enhanced_technical.get('primary_insight'):
                    primary_insight = enhanced_technical['primary_insight']
                    supporting_insights = enhanced_technical.get('supporting_insights', [])
                    
                    # Build enhanced sentiment content
                    sentiment_parts = [primary_insight]
                    
                    # Add supporting insight if it fits in character limit
                    if supporting_insights:
                        combined_length = len(primary_insight) + len(supporting_insights[0]) + 2  # +2 for ". "
                        if combined_length <= 240:  # Leave room for other content
                            sentiment_parts.append(supporting_insights[0])
                    
                    enhanced_sentiment = ". ".join(sentiment_parts)
                    logger.logger.info(f"✅ Technical signals enhanced successfully")
                    logger.logger.debug(f"Enhanced content: {enhanced_sentiment[:60]}...")
                else:
                    logger.logger.warning(f"⚠️ Technical enhancement produced no insights for {token}")
            else:
                logger.logger.debug(f"No technical analysis available for enhancement")
            ...
            # ================================================================
            # 🎯 ANALYZE SENTIMENT BASED ON AVAILABLE DATA (ENHANCED)
            # ================================================================
            
            try:
                # ================================================================
                # 📝 USE ENHANCED CONTENT OR FALLBACK TO EXISTING LOGIC
                # ================================================================
                
                if enhanced_sentiment:
                    # Use the enhanced technical content
                    sentiment = enhanced_sentiment
                    logger.logger.debug(f"Using enhanced technical sentiment")
                    
                    # For enhanced sentiment, create a simplified market context
                    market_context = f"\n{token} technical analysis for {time_desc} timeframe"
                    
                    # Add vs market context if available
                    vs_change = vs_market.get('vs_market_avg_change', 0.0)
                    if abs(vs_change) > 1.0:  # Only add if significant
                        vs_sentiment = vs_market.get('vs_market_sentiment', 'neutral')
                        if vs_sentiment in ['strongly outperforming', 'outperforming']:
                            market_context += f" (outperforming market +{abs(vs_change):.1f}%)"
                        elif vs_sentiment in ['strongly underperforming', 'underperforming']:
                            market_context += f" (underperforming market -{abs(vs_change):.1f}%)"
                            
                else:
                    # Fallback to original sentiment generation logic
                    logger.logger.debug(f"Using original sentiment generation logic")
                ...
                if trigger_type:
                    ...
                    if 'price_change' in trigger_type:
                        ...
                        # Emphasize sentiment for smart money triggers (only for non-enhanced sentiment)
                        if not enhanced_sentiment and sentiment in ["bullish", "bearish"]:
            ...
            storage_data = {
                ...
                'sentiment': {token: sentiment if not enhanced_sentiment else "enhanced_technical"},
                ...
                'enhanced_technical': enhanced_sentiment is not None,  # Track if enhancement was used
            ...
            default_storage_data = {
                ...
                'timeframe': timeframe,
                'market_context': "",
                'enhanced_technical': False,
                'timestamp': strip_timezone(datetime.now()) if hasattr(self, 'strip_timezone') else datetime.now()
            }
            return "neutral", default_storage_data

    def _get_price_history_for_analysis(self, token: str, timeframe: str, hours: int = 48) -> List[float]:
        """
        Get price history for technical analysis
        ...
        try:
            if hasattr(self, 'db') and self.db:
                ...
                price_history = self.db.build_sparkline_from_price_history(token, hours=hours)
                if price_history and len(price_history) >= 10:
                    logger.logger.debug(f"Retrieved {len(price_history)} price points for {token}")
                    return price_history
            
            logger.logger.warning(f"Insufficient price history for {token}")
            return []
            
        except Exception as e:
            logger.logger.error(f"Error getting price history for {token}: {str(e)}")
            return []
    ...
    def _validate_content_quality(self, content: str, token: str, timeframe: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            insight_indicators = [
                ...
                "machine learning", "artificial intelligence", "deep learning", "neural network", "lstm", "gru", "transformer",
                "random forest", "gradient boosting", "xgboost", "lightgbm", "catboost", "support vector machine", "svm",
                "linear regression", "logistic regression", "polynomial regression", "ridge regression", "lasso regression",
                "elastic net", "decision tree", "ensemble method", "bagging", "boosting", "stacking", "voting classifier",
                "k-means", "clustering", "classification", "regression", "supervised learning", "unsupervised learning",
                "reinforcement learning", "feature engineering", "feature selection", "dimensionality reduction", "pca",
                "cross validation", "hyperparameter tuning", "overfitting", "underfitting", "bias variance tradeoff",
                "training data", "test data", "validation data", "model accuracy", "precision", "recall", "f1 score",
                "confusion matrix", "roc curve", "auc", "loss function", "optimization", "gradient descent", "backpropagation",
                
                # Forecasting & Time Series
                "forecasting", "time series", "autoregressive", "ar model", "ma model", "arma", "arima", "sarima",
                "garch", "var model", "vector autoregression", "cointegration", "granger causality", "seasonal decomposition",
                "trend analysis", "cyclical pattern", "seasonal pattern", "stationarity", "unit root test", "dickey fuller",
                "ljung box test", "durbin watson", "autocorrelation", "partial autocorrelation", "lag", "lead indicator",
                "coincident indicator", "lagging indicator", "leading economic indicator", "predictive model", "nowcasting",
                "backtesting", "walk forward analysis", "out of sample", "in sample", "forecast accuracy", "forecast error",
                "mean absolute error", "mae", "root mean square error", "rmse", "mean absolute percentage error", "mape",
                
                # Quantitative Analysis
                "quantitative analysis", "statistical analysis", "probability", "statistical significance", "p value",
                "hypothesis testing", "confidence interval", "z score", "t test", "chi square", "anova", "regression analysis",
                "correlation analysis", "multivariate analysis", "factor analysis", "principal component analysis",
                "monte carlo simulation", "bootstrap", "jackknife", "bayesian analysis", "markov chain", "random walk",
                "brownian motion", "geometric brownian motion", "black scholes", "options pricing", "volatility modeling",
                "stochastic process", "martingale", "efficient market hypothesis", "random matrix theory", "signal processing",
                
                # Algorithmic Trading Predictions
                "algorithmic trading", "quantitative trading", "systematic trading", "automated trading", "trading algorithm",
                "trading signal", "buy signal", "sell signal", "entry signal", "exit signal", "stop loss", "take profit",
                "risk management", "position sizing", "portfolio optimization", "mean reversion strategy", "momentum strategy",
                "pairs trading", "statistical arbitrage", "market making", "high frequency trading", "latency arbitrage",
                "order flow", "market microstructure", "execution algorithm", "vwap algorithm", "twap algorithm", "iceberg order",
                "smart order routing", "dark pool", "lit market", "order book dynamics", "market impact", "slippage modeling",
                
                # Economic & Financial Predictions
                "economic forecasting", "macroeconomic indicator", "gdp growth", "inflation rate", "unemployment rate",
                "interest rate", "federal funds rate", "yield curve", "term structure", "credit spread", "risk premium",
                "economic cycle", "business cycle", "recession prediction", "expansion", "contraction", "peak", "trough",
                "leading indicator", "composite index", "consumer confidence", "business sentiment", "purchasing managers index",
                "pmi", "economic surprise index", "oecd cli", "nowcast", "central bank policy", "monetary policy",
                "fiscal policy", "inflation expectation", "real interest rate", "exchange rate", "currency forecast",
                
                # Market Prediction Models
                "market prediction", "price prediction", "trend prediction", "direction prediction", "magnitude prediction",
                "volatility prediction", "volume prediction", "sentiment prediction", "regime prediction", "crash prediction",
                "bubble detection", "anomaly detection", "outlier detection", "change point detection", "structural break",
                "market timing", "tactical asset allocation", "strategic asset allocation", "risk parity", "factor investing",
                "alternative data", "satellite data", "social media sentiment", "news sentiment", "earnings surprise",
                "analyst estimates", "earnings revision", "guidance", "forward looking", "predictive analytics",
                
                # Crypto-Specific Predictions
                "crypto prediction", "blockchain analysis", "onchain prediction", "network prediction", "adoption forecast",
                "hash rate prediction", "difficulty prediction", "mining profitability", "validator behavior", "staking yield",
                "defi yield prediction", "tvl forecast", "token unlock schedule", "vesting schedule", "burn rate prediction",
                "inflation schedule", "halving effect", "cycle analysis", "market cycle prediction", "altcoin season",
                "bitcoin dominance", "correlation prediction", "cross-chain analysis", "layer 2 adoption", "scaling solution",
                "governance prediction", "dao voting", "proposal outcome", "network upgrade", "fork prediction",

                # Fundamental Analysis
                "fundamental analysis", "intrinsic value", "fair value", "book value", "market value", "enterprise value",
                "price to earnings", "p/e ratio", "price to book", "p/b ratio", "price to sales", "p/s ratio",
                "enterprise value to ebitda", "ev/ebitda", "price earnings growth", "peg ratio", "debt to equity",
                "current ratio", "quick ratio", "cash ratio", "return on equity", "roe", "return on assets", "roa",
                "return on invested capital", "roic", "gross margin", "operating margin", "net margin", "ebitda margin",
                "free cash flow", "cash flow analysis", "discounted cash flow", "dcf", "net present value", "npv",
                "internal rate of return", "irr", "weighted average cost of capital", "wacc", "cost of equity",
                "beta", "alpha", "sharpe ratio", "sortino ratio", "information ratio", "treynor ratio", "jensen alpha",
                
                # Technical Analysis Deep Dive
                "technical analysis", "chart analysis", "pattern recognition", "trend analysis", "support level",
                "resistance level", "breakout analysis", "breakdown analysis", "volume analysis", "momentum analysis",
                "oscillator analysis", "divergence analysis", "convergence analysis", "multi timeframe analysis",
                "intermarket analysis", "sector rotation", "relative strength analysis", "comparative analysis",
                "japanese candlestick", "candlestick pattern", "chart pattern", "harmonic analysis", "elliott wave analysis",
                "fibonacci analysis", "gann analysis", "wyckoff analysis", "market profile analysis", "auction theory",
                "supply demand analysis", "smart money analysis", "institutional analysis", "retail sentiment",
                "contrarian analysis", "consensus analysis", "positioning analysis", "flow analysis", "commitment of traders",
                
                # Risk Analysis
                "risk analysis", "risk assessment", "risk management", "value at risk", "var", "conditional var", "cvar",
                "expected shortfall", "maximum drawdown", "volatility analysis", "correlation analysis", "beta analysis",
                "systematic risk", "unsystematic risk", "idiosyncratic risk", "market risk", "credit risk",
                "liquidity risk", "operational risk", "model risk", "tail risk", "fat tail", "extreme value theory",
                "stress testing", "scenario analysis", "sensitivity analysis", "monte carlo analysis", "historical simulation",
                "parametric var", "non parametric var", "portfolio risk", "concentration risk", "diversification analysis",
                
                # Quantitative Analysis Methods
                "statistical analysis", "econometric analysis", "multivariate analysis", "factor analysis", "cluster analysis",
                "discriminant analysis", "canonical correlation", "structural equation modeling", "time series analysis",
                "panel data analysis", "cross sectional analysis", "longitudinal analysis", "survival analysis",
                "event study", "cointegration analysis", "causality analysis", "impulse response", "variance decomposition",
                "regime switching", "threshold analysis", "nonlinear analysis", "chaos theory", "fractal analysis",
                "wavelets", "spectral analysis", "frequency domain", "filtering", "smoothing", "seasonal adjustment",
                
                # Market Structure Analysis
                "market structure", "market efficiency", "market liquidity", "bid ask spread", "market depth",
                "order flow analysis", "trade classification", "market impact analysis", "execution quality",
                "best execution", "transaction cost analysis", "implementation shortfall", "market making analysis",
                "adverse selection", "inventory risk", "information asymmetry", "price discovery", "volatility clustering",
                "microstructure noise", "tick size", "lot size", "trading halt", "circuit breaker", "market fragmentation",
                "dark pool analysis", "lit market analysis", "crossing network", "alternative trading system",
                
                # Behavioral Analysis
                "behavioral analysis", "behavioral finance", "market psychology", "investor sentiment", "crowd behavior",
                "herd mentality", "fear greed index", "put call ratio", "volatility index", "sentiment indicators",
                "survey data", "insider trading", "institutional flow", "smart money", "dumb money", "momentum investing",
                "value investing", "growth investing", "contrarian investing", "disposition effect", "anchoring bias",
                "confirmation bias", "overconfidence bias", "recency bias", "availability heuristic", "representativeness heuristic",
                "loss aversion", "prospect theory", "mental accounting", "framing effect", "endowment effect",
                
                # Alternative Data Analysis
                "alternative data", "satellite imagery", "geolocation data", "credit card transactions", "app usage",
                "web scraping", "social media analysis", "news analysis", "earnings call analysis", "sec filing analysis",
                "patent analysis", "job posting analysis", "supply chain analysis", "weather data", "economic tracking",
                "nowcasting", "high frequency indicators", "real time data", "unstructured data", "text mining",
                "natural language processing", "nlp", "sentiment scoring", "topic modeling", "entity recognition",
                "network analysis", "graph theory", "centrality measures", "community detection", "link analysis",
                
                # Crypto-Specific Analysis
                "onchain analysis", "blockchain analysis", "network analysis", "address analysis", "transaction analysis",
                "whale analysis", "exchange flow", "miner analysis", "staking analysis", "governance analysis",
                "tokenomics analysis", "supply analysis", "demand analysis", "utility analysis", "adoption analysis",
                "developer activity", "network growth", "active addresses", "transaction volume", "network value",
                "realized cap", "mvrv ratio", "nvt ratio", "stock to flow", "puell multiple", "realized price",
                "hodl waves", "coin days destroyed", "spent output profit ratio", "sopr", "long term holder",
                "short term holder", "exchange netflow", "stable coin analysis", "defi analysis", "yield analysis",
                "liquidity analysis", "impermanent loss", "protocol revenue", "fee analysis", "burn analysis"
            ]
    ...
    def _enhance_technical_signals(self, technical_data: Dict[str, Any], token: str, timeframe: str) -> Dict[str, Any]:
        """
        Enhance raw technical indicators with specific, engaging language
        
        This method processes technical indicators BEFORE they become generic content,
        converting raw values into meaningful, context-specific insights.
        
        Args:
            technical_data (Dict[str, Any]): Raw technical analysis data from prediction engine
            token (str): Token symbol for context-specific language
            timeframe (str): Timeframe for appropriate technical focus
            
        Returns:
            Dict[str, Any]: Enhanced technical data with improved content
        """
        logger.logger.debug(f"🚀 TECHNICAL SIGNAL ENHANCEMENT: {token} ({timeframe})")
        logger.logger.debug(f"📊 Input technical data keys: {list(technical_data.keys()) if technical_data else 'None'}")
        
        try:
            # Create enhanced copy of technical data
            enhanced_data = technical_data.copy() if technical_data else {}
            
            # Extract indicators from the technical analysis structure
            indicators = technical_data.get('indicators', {})
            signals = technical_data.get('signals', {})
            overall_signal = technical_data.get('overall_signal', 'neutral')
            trend = technical_data.get('overall_trend', 'neutral')
            
            logger.logger.debug(f"📈 Processing indicators: {list(indicators.keys())}")
            ...
            if isinstance(rsi_value, (int, float)):
                rsi_enhanced = self._enhance_rsi_signal(rsi_value, token, timeframe)
                if rsi_enhanced:
                    enhanced_data['rsi_insight'] = rsi_enhanced
                    logger.logger.debug(f"✅ RSI enhanced: {rsi_enhanced[:50]}...")
            ...
            if isinstance(macd_data, dict) and macd_data:
                macd_enhanced = self._enhance_macd_signal(macd_data, token, timeframe)
                if macd_enhanced:
                    enhanced_data['macd_insight'] = macd_enhanced
                    logger.logger.debug(f"✅ MACD enhanced: {macd_enhanced[:50]}...")
            ...
            if isinstance(bb_data, dict) and bb_data:
                bb_enhanced = self._enhance_bollinger_signal(bb_data, token, timeframe)
                if bb_enhanced:
                    enhanced_data['bollinger_insight'] = bb_enhanced
                    logger.logger.debug(f"✅ Bollinger Bands enhanced: {bb_enhanced[:50]}...")
            
            # ================================================================
            # 📈 VOLUME ENHANCEMENT
            # ================================================================
            
            volume_data = indicators.get('obv') or indicators.get('volume')
            if volume_data is not None:
                volume_enhanced = self._enhance_volume_signal(volume_data, token, timeframe)
                if volume_enhanced:
                    enhanced_data['volume_insight'] = volume_enhanced
                    logger.logger.debug(f"✅ Volume enhanced: {volume_enhanced[:50]}...")
            ...
            if isinstance(trend_strength, (int, float)):
                trend_enhanced = self._enhance_trend_signal(trend, trend_strength, token, timeframe)
                if trend_enhanced:
                    enhanced_data['trend_insight'] = trend_enhanced
                    logger.logger.debug(f"✅ Trend enhanced: {trend_enhanced[:50]}...")
            
            # ================================================================
            # 🎯 SIGNAL CONFLUENCE ENHANCEMENT
            # ================================================================
            
            confluence_factors = technical_data.get('confluence_analysis', {}).get('confluence_factors', 0)
            if confluence_factors > 0:
                confluence_enhanced = self._enhance_confluence_signal(confluence_factors, overall_signal, token, timeframe)
                if confluence_enhanced:
                    enhanced_data['confluence_insight'] = confluence_enhanced
                    logger.logger.debug(f"✅ Confluence enhanced: {confluence_enhanced[:50]}...")
            
            # ================================================================
            # 📝 CREATE ENHANCED CONTENT SUMMARY (FIXED)
            # ================================================================

            enhanced_insights = []
            insight_keys = ['rsi_insight', 'macd_insight', 'bollinger_insight', 'volume_insight', 'trend_insight', 'confluence_insight']

            for key in insight_keys:
                if key in enhanced_data and enhanced_data[key]:
                    enhanced_insights.append(enhanced_data[key])

            # Create prioritized content based on timeframe
            if enhanced_insights:
                # ONLY use primary insight - don't duplicate with supporting insights
                enhanced_data['primary_insight'] = self._select_primary_insight(enhanced_insights, timeframe)
                
                # Remove supporting_insights to prevent duplication
                # enhanced_data['supporting_insights'] = enhanced_insights[:2]  # REMOVE THIS LINE
                
                logger.logger.info(f"✅ TECHNICAL ENHANCEMENT SUCCESS:")
                logger.logger.debug(f"   • Enhanced insights: {len(enhanced_insights)}")
                logger.logger.debug(f"   • Primary insight: {enhanced_data['primary_insight'][:60]}...")
            else:
                logger.logger.warning(f"⚠️ No insights enhanced for {token}")

            return enhanced_data
            
        except Exception as e:
            logger.logger.error(f"❌ TECHNICAL ENHANCEMENT FAILED: {str(e)}")
            logger.logger.debug(f"🔍 Error details: {e.__class__.__name__}: {str(e)}")
            return technical_data  # Return original data on error

    def _enhance_rsi_signal(self, rsi_value: float, token: str, timeframe: str) -> str:
        """
        Convert RSI value into specific, engaging content
        ...
        Returns:
            Enhanced RSI insight string
        """
        try:
            if rsi_value <= 20:
                return f"{token} showing extreme oversold conditions at RSI {rsi_value:.1f} - potential bounce territory"
            elif rsi_value <= 30:
                return f"{token} RSI at {rsi_value:.1f} suggests oversold momentum - accumulation zone active"
            elif rsi_value <= 40:
                return f"{token} momentum weakening with RSI {rsi_value:.1f} - support levels critical"
            elif rsi_value >= 80:
                return f"{token} RSI hits {rsi_value:.1f} indicating extreme overbought pressure - correction risk elevated"
            elif rsi_value >= 70:
                return f"{token} entering overbought territory at RSI {rsi_value:.1f} - momentum peak signals"
            elif rsi_value >= 60:
                return f"{token} building bullish momentum with RSI {rsi_value:.1f} - uptrend acceleration"
            else:
                return f"{token} maintaining neutral RSI balance at {rsi_value:.1f} - range-bound consolidation"
                
        except Exception as e:
            logger.logger.debug(f"RSI enhancement error: {e}")
            return ""

    def _enhance_macd_signal(self, macd_data: Dict[str, Any], token: str, timeframe: str) -> str:
        """
        Convert MACD data into specific, engaging content
        ...
        Returns:
            Enhanced MACD insight string
        """
        try:
            macd_line = macd_data.get('macd_line', macd_data.get('macd', 0))
            signal_line = macd_data.get('signal_line', macd_data.get('signal', 0))
            histogram = macd_data.get('histogram', 0)
            
            # Convert to float for calculations
            macd_line = float(macd_line) if macd_line is not None else 0
            signal_line = float(signal_line) if signal_line is not None else 0
            histogram = float(histogram) if histogram is not None else 0
            
            if macd_line > signal_line and histogram > 0:
                if histogram > abs(macd_line * 0.1):  # Strong positive histogram
                    return f"{token} MACD bullish crossover strengthening - momentum expansion confirmed"
                else:
                    return f"{token} MACD above signal line - early bullish momentum developing"
            elif macd_line < signal_line and histogram < 0:
                if abs(histogram) > abs(macd_line * 0.1):  # Strong negative histogram
                    return f"{token} MACD bearish divergence intensifying - downward pressure mounting"
                else:
                    return f"{token} MACD below signal line - bearish momentum emerging"
            elif abs(macd_line - signal_line) < abs(macd_line * 0.05):  # Lines very close
                return f"{token} MACD convergence zone - directional breakout imminent"
            else:
                return f"{token} MACD showing mixed momentum signals - consolidation phase active"
                
        except Exception as e:
            logger.logger.debug(f"MACD enhancement error: {e}")
            return ""

    def _enhance_bollinger_signal(self, bb_data: Dict[str, Any], token: str, timeframe: str) -> str:
        """
        Convert Bollinger Bands data into specific, engaging content
        
        Args:
            bb_data: Bollinger Bands indicator data
            token: Token symbol
            timeframe: Analysis timeframe
            
        Returns:
            Enhanced Bollinger Bands insight string
        """
        try:
            upper = float(bb_data.get('upper', 0))
            middle = float(bb_data.get('middle', 0))  
            lower = float(bb_data.get('lower', 0))
            position = float(bb_data.get('position', 50))  # Position within bands
            
            if position >= 90:
                return f"{token} hugging upper Bollinger Band - extreme bullish pressure, breakout potential"
            elif position >= 75:
                return f"{token} approaching upper Bollinger resistance - overbought momentum building"
            elif position <= 10:
                return f"{token} testing lower Bollinger support - oversold bounce opportunity"
            elif position <= 25:
                return f"{token} near lower Bollinger Band - accumulation zone activation"
            elif 45 <= position <= 55:
                # Check band width for squeeze
                if upper > 0 and lower > 0:
                    band_width = (upper - lower) / middle * 100
                    if band_width < 10:  # Narrow bands indicate squeeze
                        return f"{token} Bollinger Band squeeze active - explosive breakout preparation"
                    else:
                        return f"{token} balanced within Bollinger mid-range - neutral momentum zone"
            
            return f"{token} Bollinger Band positioning at {position:.1f}% - trend direction developing"
            
        except Exception as e:
            logger.logger.debug(f"Bollinger Bands enhancement error: {e}")
            return ""

    def _enhance_volume_signal(self, volume_data: Any, token: str, timeframe: str) -> str:
        """
        Convert volume data into specific, engaging content
        
        Args:
            volume_data: Volume indicator data (OBV or raw volume)
            token: Token symbol
            timeframe: Analysis timeframe
            
        Returns:
            Enhanced volume insight string
        """
        try:
            # Handle different volume data types
            if isinstance(volume_data, dict):
                volume_value = volume_data.get('value', volume_data.get('obv', 0))
            else:
                volume_value = float(volume_data) if volume_data is not None else 0
                
            volume_value = float(volume_value)
            
            # Determine volume significance based on magnitude and timeframe
            if timeframe == "1h":
                if volume_value > 1000000:
                    return f"{token} {timeframe} volume surge indicates institutional participation"
                elif volume_value > 500000:
                    return f"{token} elevated {timeframe} volume confirms directional conviction"
                else:
                    return f"{token} standard {timeframe} volume suggests retail-driven movement"
            elif timeframe == "24h":
                if volume_value > 5000000:
                    return f"{token} exceptional daily volume signals major market interest"
                elif volume_value > 2000000:
                    return f"{token} strong daily volume supports trend continuation"
                else:
                    return f"{token} moderate daily volume indicates consolidation phase"
            else:  # 7d
                return f"{token} weekly volume patterns suggest {('accumulation' if volume_value > 0 else 'distribution')} phase"
                
        except Exception as e:
            logger.logger.debug(f"Volume enhancement error: {e}")
            return ""

    def _enhance_trend_signal(self, trend: str, trend_strength: float, token: str, timeframe: str) -> str:
        """
        Convert trend data into specific, engaging content
        
        Args:
            trend: Trend direction ('bullish', 'bearish', 'neutral')
            trend_strength: Trend strength value (0-100)
            token: Token symbol
            timeframe: Analysis timeframe
            
        Returns:
            Enhanced trend insight string
        """
        try:
            ...
            if trend == 'bullish':
                if trend_strength >= 80:
                    return f"{token} strong bullish trend with {trend_strength:.1f}% confidence - upward trajectory sustained"
                elif trend_strength >= 60:
                    return f"{token} moderate bullish bias at {trend_strength:.1f}% strength - positive momentum developing"
                else:
                    return f"{token} weak bullish lean with {trend_strength:.1f}% conviction - early stage recovery"
            elif trend == 'bearish':
                if trend_strength >= 80:
                    return f"{token} strong bearish trend at {trend_strength:.1f}% confidence - downward pressure dominant"
                elif trend_strength >= 60:
                    return f"{token} moderate bearish bias with {trend_strength:.1f}% strength - corrective phase active"
                else:
                    return f"{token} weak bearish tilt at {trend_strength:.1f}% conviction - minor pullback territory"
            else:  # neutral
                if trend_strength <= 30:
                    return f"{token} neutral trend with low {trend_strength:.1f}% conviction - directional uncertainty"
                else:
                    return f"{token} balanced trend dynamics at {trend_strength:.1f}% - consolidation range established"
                    
        except Exception as e:
            logger.logger.debug(f"Trend enhancement error: {e}")
            return ""

    def _enhance_confluence_signal(self, confluence_factors: int, overall_signal: str, token: str, timeframe: str) -> str:
        """
        Convert confluence data into specific, engaging content
        
        Args:
            confluence_factors: Number of confirming indicators
            overall_signal: Overall signal direction
            token: Token symbol
            timeframe: Analysis timeframe
            
        Returns:
            Enhanced confluence insight string
        """
        try:
            confluence_factors = int(confluence_factors)
            
            if confluence_factors >= 5:
                return f"{token} {confluence_factors} indicators align {overall_signal} - high-conviction signal convergence"
            elif confluence_factors >= 3:
                return f"{token} {confluence_factors} technical factors confirm {overall_signal} bias - moderate confidence setup"
            elif confluence_factors >= 2:
                return f"{token} {confluence_factors} indicators suggest {overall_signal} tendency - early signal formation"
            else:
                return f"{token} limited technical confluence - signal development pending"
                
        except Exception as e:
            logger.logger.debug(f"Confluence enhancement error: {e}")
            return ""

    def _select_primary_insight(self, insights: List[str], timeframe: str) -> str:
        """
        Select the most relevant insight based on timeframe priority
        
        Args:
            insights: List of enhanced insight strings
            timeframe: Analysis timeframe for priority selection
            
        Returns:
            Primary insight string
        """
        try:
            if not insights:
                return ""
                
            # Priority order by timeframe
            timeframe_priorities = {
                "1h": ["rsi", "macd", "confluence", "bollinger", "volume", "trend"],
                "24h": ["trend", "confluence", "bollinger", "macd", "volume", "rsi"],
                "7d": ["trend", "confluence", "volume", "bollinger", "macd", "rsi"]
            }
            
            priority_order = timeframe_priorities.get(timeframe, timeframe_priorities["1h"])
            
            # Find insight that matches highest priority
            for priority_keyword in priority_order:
                for insight in insights:
                    if priority_keyword.lower() in insight.lower():
                        return insight
            
            # Fallback to first insight if no priority match
            return insights[0]
            
        except Exception as e:
            logger.logger.debug(f"Primary insight selection error: {e}")
    ...
    def start(self) -> None:
        ...
        Enhanced with data collection mode when insufficient data is available
        """
        try:
            retry_count = 0
            max_setup_retries = 3
            data_collection_mode = False  # Flag to track if we're in data collection only mode
            ...
            if market_data:
                ...
                try:
                    ...
                    database_tokens = self.get_tokens_with_recent_data_by_market_cap(hours=24, limit=25)
                    if not database_tokens:
                        logger.logger.warning("⚠️ NO RECENT DATA: No tokens from database - entering data collection mode")
                        ...
                        if not available_tokens:
                            logger.logger.warning("⚠️ NO COMPATIBLE DATA: No database tokens found in market_data - entering data collection mode")
                            data_collection_mode = True
                        else:
                            ...
                            logger.logger.info("✅ Prediction queueing completed successfully")
                            
                except Exception as e:
                    logger.logger.warning(f"⚠️ DATABASE ERROR: Database token selection failed: {str(e)} - entering data collection mode")
                    data_collection_mode = True
            else:
                logger.logger.warning("⚠️ NO MARKET DATA: Unable to fetch market data - entering data collection mode")
                data_collection_mode = True

            # Log current mode
            if data_collection_mode:
                logger.logger.info("🔄 ENTERING DATA COLLECTION MODE")
                logger.logger.info("📊 Bot will continue running and attempt to collect new data")
                logger.logger.info("🚫 Trading and prediction operations will be skipped until sufficient data is available")
            else:
                logger.logger.info("✅ ENTERING FULL OPERATION MODE")
                logger.logger.info("🎯 Bot will perform analysis, predictions, and posting operations")

            # Main execution loop
            while True:
                try:
                    # Check if we can exit data collection mode
                    if data_collection_mode:
                        logger.logger.info("🔍 DATA COLLECTION MODE: Attempting to collect fresh data...")
                        
                        # 🛡️ CIRCUIT BREAKER CHECK - PREVENT INFINITE API LOOPS
                        if not self.data_fetch_circuit_breaker.can_attempt_fetch():
                            logger.logger.info("🛡️ Data fetch blocked by circuit breaker - sleeping 60s")
                            time.sleep(60)
                            continue
                        
                        try:
                            # Try to fetch new market data
                            fresh_market_data = self._get_crypto_data()
                            
                            if fresh_market_data:
                                # 🛡️ SUCCESS: Record successful fetch
                                self.data_fetch_circuit_breaker.record_success()
                                
                                # Check if we now have tokens with recent data
                                try:
                                    fresh_database_tokens = self.get_tokens_with_recent_data_by_market_cap(hours=24, limit=25)
                                    if fresh_database_tokens:
                                        available_tokens = [token for token in fresh_database_tokens if token in fresh_market_data]
                                        if available_tokens:
                                            logger.logger.info("✅ DATA COLLECTION SUCCESS: Found tokens with recent data!")
                                            logger.logger.info(f"📊 Available tokens: {available_tokens[:5]}...")
                                            
                                            # Queue predictions for the newly available tokens
                                            top_tokens = self._prioritize_tokens(available_tokens, fresh_market_data)[:5]
                                            logger.logger.info(f"🎯 Queueing predictions for: {', '.join(top_tokens)}")
                                            for token in top_tokens:
                                                self._queue_predictions_for_all_timeframes(token, fresh_market_data)
                                            
                                            # Exit data collection mode
                                            data_collection_mode = False
                                            logger.logger.info("🚀 SWITCHING TO FULL OPERATION MODE")
                                        else:
                                            logger.logger.info("📊 Data collection: Tokens found in database but not in market data")
                                    else:
                                        logger.logger.info("📊 Data collection: No tokens with recent data yet")
                                except Exception as collection_error:
                                    logger.logger.warning(f"⚠️ Data collection check failed: {str(collection_error)}")
                            else:
                                # 🛡️ FAILURE: No market data returned
                                self.data_fetch_circuit_breaker.record_failure("no_market_data")
                                logger.logger.info("📊 Data collection: Unable to fetch market data")
                        
                        except Exception as fetch_error:
                            # 🛡️ FAILURE: Exception during fetch
                            error_msg = str(fetch_error)
                            if "429" in error_msg or "rate limit" in error_msg.lower():
                                self.data_fetch_circuit_breaker.record_failure("rate_limit_429")
                                logger.logger.error(f"🛡️ Rate limit error caught by circuit breaker: {error_msg}")
                            else:
                                self.data_fetch_circuit_breaker.record_failure("api_exception")
                                logger.logger.error(f"🛡️ API error caught by circuit breaker: {error_msg}")
                        
                        # In data collection mode, run limited operations
                        if data_collection_mode:
                            logger.logger.info("🔄 DATA COLLECTION MODE: Skipping analysis cycle, continuing data collection...")
                            # Still attempt to run basic operations to maintain connection
                            time.sleep(300)  # Wait 5 minutes before next data collection attempt
                            continue

                    # Run normal analysis cycle (only when not in data collection mode)
                    self._run_analysis_cycle()
                    
                    # Calculate sleep time until next regular check
                    time_since_last = safe_datetime_diff(datetime.now(), self.last_reply_check)
                    
                    # Use reply_check_interval converted to seconds (it's stored in minutes)
                    check_interval_seconds = self.reply_check_interval * 60
                    ...
                    if time_since_last >= check_interval_seconds:
                        ...
                        sleep_time = check_interval_seconds - time_since_last
                    
                    # Log next check info
                    if sleep_time > 0:
                        next_check_time = datetime.now() + timedelta(seconds=sleep_time)
                        logger.logger.info(f"⏰ Next check in {sleep_time:.0f}s at {next_check_time.strftime('%H:%M:%S')}")
                        time.sleep(sleep_time)
                    ...
                    logger.log_error("Main Loop Error", str(e))
                    logger.logger.warning(f"⚠️ Error in main loop: {str(e)} - continuing operation...")
                    time.sleep(60)  # Wait 1 minute before retrying
                    
        except Exception as e:
            logger.log_error("Bot Start Critical Error", str(e))
            logger.logger.error(f"❌ CRITICAL ERROR: {str(e)}")
        finally:
            # Cleanup
            logger.logger.info("🧹 Cleaning up bot resources...")
            self.prediction_thread_running = False
...
if __name__ == "__main__":
    try:
        ...
        from utils.logger import logger
        logger.logger.info("🔍 Setting up bot method monitoring...")
        monitored_bot, monitor = setup_bot_monitoring(bot)
        logger.logger.info("✅ Bot monitoring active - all method calls will be tracked")

        # Start the MONITORED bot (CORRECTED)
        monitored_bot.start()  # ← CHANGED: Use monitored_bot instead of bot
