...
"""
    ...
    def find_market_related_posts(self, posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Find market-related posts with improved recognition capabilities
    
        Args:
            posts: List of post data dictionaries
        
        Returns:
            List of market-related posts
        """
        market_related = []
    
        # Broader set of market-related terms
        market_terms = [
            # Core crypto terms
            'crypto', 'bitcoin', 'btc', 'eth', 'ethereum', 'blockchain', 'token', 'coin', 
            # Market action terms
            'price', 'bull', 'bear', 'market', 'trade', 'trading', 'chart', 'analysis',
            'buy', 'sell', 'pump', 'dump', 'moon', 'dip', 'crash', 'rally', 'correction',
            # Technical terms
            'support', 'resistance', 'trend', 'breakout', 'consolidation', 'volume', 'candle',
            # Exchange terms
            'exchange', 'binance', 'coinbase', 'kraken', 'huobi', 'kucoin', 'ftx', 'dex', 'cex',
            # DeFi terms
            'defi', 'yield', 'farm', 'stake', 'staking', 'liquidity', 'amm', 'dao', 'governance',
            # NFT terms
            'nft', 'collectible',
            # Misc relevant terms
            'wallet', 'address', 'key', 'mining', 'hash', 'block', 'node'
        ]
    
        # Popular token symbols
        token_symbols = [
            'btc', 'eth', 'sol', 'xrp', 'ada', 'dot', 'doge', 'avax', 'bnb', 
            'matic', 'link', 'uni', 'aave', 'mkr', 'crv', 'sushi', 'ltc', 'etc'
        ]
    
        # Token names
        token_names = [
            'bitcoin', 'ethereum', 'solana', 'ripple', 'cardano', 'polkadot', 'avalanche',
            'dogecoin', 'binance', 'polygon', 'chainlink', 'uniswap', 'aave', 'maker',
            'curve', 'sushiswap', 'litecoin'
        ]
    
        # Check each post for market-related content
        for post in posts:
            text = post.get('text', '').lower()
        
            # Check for market terms
            market_term_matches = sum(1 for term in market_terms if term in text)
        
            # Check for token symbols (with word boundaries to avoid false matches)
            token_symbol_matches = sum(1 for symbol in token_symbols 
                                     if re.search(r'\b' + symbol + r'\b', text) 
                                     or f'${symbol}' in text)
        
            # Check for token names
            token_name_matches = sum(1 for name in token_names if name in text)
        
            # Determine if post is market-related based on matches
            # More lenient criteria: either sufficient market terms OR any token mention
            is_market_related = (
                market_term_matches >= 2 or  # At least 2 market terms
                token_symbol_matches >= 1 or  # At least 1 token symbol
                token_name_matches >= 1 or  # At least 1 token name
                (market_term_matches >= 1 and (token_symbol_matches >= 1 or token_name_matches >= 1))  # At least 1 market term AND 1 token
            )
        
            # Add sentiment detection for filter refinement
            sentiment = 'neutral'
            sentiment_score = 0
        
            # Basic sentiment detection
            positive_terms = ['bull', 'bullish', 'green', 'up', 'moon', 'pump', 'long', 'profit', 'gain']
            negative_terms = ['bear', 'bearish', 'red', 'down', 'dump', 'crash', 'short', 'loss', 'pain']
            ...
            # Add market sentiment to post data
            post['market_sentiment'] = sentiment
            post['sentiment_score'] = sentiment_score
        
            # Also check for questions which are good reply opportunities
            has_question = '?' in text
            post['has_question'] = has_question
        
            # Mark as market-related or adjust score based on additional factors
            if is_market_related:
                post['is_market_related'] = True
            
                # Boost reply worthiness for posts with questions
                if has_question:
                    post['reply_priority'] = 'high'
            
                # Boost reply worthiness for posts with strong sentiment
                elif sentiment_score >= 2:
                    post['reply_priority'] = 'medium'
                else:
                    post['reply_priority'] = 'normal'
                
                market_related.append(post)
            
            # Add posts with questions about crypto even if not strictly market-related
            elif has_question and (token_symbol_matches > 0 or token_name_matches > 0 or market_term_matches > 0):
                post['is_market_related'] = False
                post['is_crypto_question'] = True
                post['reply_priority'] = 'high'
                market_related.append(post)
