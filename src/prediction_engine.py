...
class EnhancedPredictionEngine:
    ...
    def __init__(self, database, llm_provider, config=None, bot=None):
        """
    ...
    def _assemble_trading_prediction(self, trading_signal: Dict[str, Any], technical_signals: Dict[str, Any], prediction_start_time: float) -> Dict[str, Any]:
        ...
        try:
            ...
            prediction = {
                ...
                'price': round(take_profit_price, 6),  # Use take_profit_price that's already defined
                'lower_bound': round(lower_bound_price, 6),  # Use lower_bound_price that's already calculated
                'upper_bound': round(upper_bound_price, 6),  # Use upper_bound_price that's already calculated
                'percent_change': round(expected_return_pct, 2),  # Use expected_return_pct that's already defined
    ...
    def _combine_predictions_without_llm(self, token: str, current_price: float, 
                ...
                """
                Combine predictions from multiple models without LLM enhancement
                Creates a database-compatible prediction with all required fields
                ...
                Returns:
                    Combined prediction with database compatibility
                """
                logger.logger.info(f"🔄 Combining predictions without LLM for {token} ({timeframe})")
                
                # Calculate weighted average prediction price
                # Weight technical analysis higher for shorter timeframes
                if timeframe == "1h":
                    tech_weight = 0.5
                    stat_weight = 0.2
                    ml_weight = 0.3
                ...
                # Extract prices with fallbacks to current price
                ...
                price = (
                    tech_price * tech_weight +
                    stat_price * stat_weight +
                    ml_price * ml_weight
                )
                
                # Calculate percent change
                percent_change = ((price / current_price) - 1) * 100
                
                # Set confidence based on agreement between models
                # Higher confidence if models agree, lower if they diverge
                price_variance = max(
                    abs(tech_price - stat_price) / current_price,
                    abs(tech_price - ml_price) / current_price,
                    abs(stat_price - ml_price) / current_price
                ) * 100  # Convert to percentage
                
                # Base confidence on inverse of variance (models agree = higher confidence)
                if price_variance < 1:
                    confidence = 85.0
                elif price_variance < 2:
                    confidence = 75.0
                elif price_variance < 4:
                    confidence = 65.0
                else:
                    confidence = 55.0
                
                # Get confidence from individual models
                tech_conf = technical_prediction.get('confidence', 70.0)
                stat_conf = statistical_prediction.get('confidence', 70.0)
                ml_conf = ml_prediction.get('confidence', 70.0)
                
                # Fine-tune confidence based on individual model confidence
                avg_model_conf = (tech_conf * tech_weight + stat_conf * stat_weight + ml_conf * ml_weight)
                confidence = (confidence + avg_model_conf) / 2
                
                # Calculate bounds based on confidence
                confidence_factor = confidence / 100.0
                base_range = 2.0  # Base 2% range
                adjusted_range = base_range * (1 - confidence_factor * 0.5)  # Tighter range with higher confidence
                
                lower_bound = price * (1 - adjusted_range / 100)
                upper_bound = price * (1 + adjusted_range / 100)
                
                # Determine sentiment based on percent change
                ...
                if sentiment == "BULLISH" and confidence > 65:
                    action = "BUY"
                elif sentiment == "BEARISH" and confidence > 65:
                    action = "SELL"
                else:
                    action = "HOLD"
                
                # Collect key factors from all models
                key_factors = []
                if 'key_factors' in technical_prediction and isinstance(technical_prediction['key_factors'], list):
                    key_factors.extend(technical_prediction['key_factors'][:1])  # Take top factor from technical
                if 'key_factors' in statistical_prediction and isinstance(statistical_prediction['key_factors'], list):
                    key_factors.extend(statistical_prediction['key_factors'][:1])  # Take top factor from statistical
                if 'key_factors' in ml_prediction and isinstance(ml_prediction['key_factors'], list):
                    key_factors.extend(ml_prediction['key_factors'][:1])  # Take top factor from ML
                
                # Ensure we have at least some key factors
                if not key_factors:
                    key_factors = ["Technical analysis", "Statistical models", "Market conditions"]
                ...
                rationale = f"Combined analysis of technical indicators, statistical models, and machine learning predictions shows {direction} momentum for {token} over the next {timeframe}. Market condition is {market_condition} with {momentum} signals from {key_factors[0].lower()}."
                
                # ================================================================
                # 🎯 ASSEMBLE COMPLETE DATABASE-COMPATIBLE PREDICTION 🎯
                # ================================================================
                
                prediction = {
                    # Core fields for database compatibility
                    "price": round(price, 6),
                    "confidence": round(confidence, 1),
                    ...
                    "stop_loss": round(lower_bound, 6),
                    "take_profit": round(upper_bound, 6),
                    "position_size": round(confidence / 2, 1),  # Position size based on confidence
                    
                    # Market context
                    "current_price": round(current_price, 6),
                    "predicted_price": round(price, 6),
                    "volatility_score": round(statistical_prediction.get('volatility_score', 50.0), 1),
                    "market_condition": market_condition,
                    "direction": sentiment.lower(),
                    ...
                    "technical_score": round(technical_prediction.get('technical_score', 50.0), 1),
                    "trend_strength": round(technical_prediction.get('trend_strength', 50.0), 1),
                ...
                logger.logger.info(
                    f"✅ Combined prediction for {token} ({timeframe}): "
                    f"${price:.6f} ({percent_change:+.2f}%), "
                    f"Confidence: {confidence:.1f}%, "
                    f"Action: {action}"
                )
    ...
    def _generate_llm_prediction(self, token: str, prices: List[float], volumes: List[float], 
        ...
        Args:
            token: Token symbol
            prices: Historical price data
            volumes: Historical volume data (can be empty)
            current_price: Current price
            technical_prediction: Technical analysis prediction
            statistical_prediction: Statistical models prediction
            ml_prediction: Machine learning prediction
        ...
        Returns:
            LLM-enhanced prediction result with database compatibility
        """
        # Safety check for volumes - ensure it's a list with at least one element
        if not isinstance(volumes, list) or len(volumes) == 0:
            volumes = [0] * len(prices) if isinstance(prices, list) and len(prices) > 0 else [0]
        
        # Check if LLM provider is available
        if not self.llm_provider:
            logger.logger.info(f"LLM provider not available for {token} prediction")
            # Create a weighted average prediction with existing data
            price = (
                technical_prediction.get('price', current_price) * 0.4 +
                statistical_prediction.get('price', current_price) * 0.3 +
                ml_prediction.get('price', current_price) * 0.3
            )
            ...
            # Create prediction with direct database compatibility
            prediction = {
                # Core prediction fields at root level for database compatibility
                "price": round(price, 6),
                "confidence": round(confidence, 1),
                "lower_bound": round(price * 0.98, 6),
                "upper_bound": round(price * 1.02, 6),
                "percent_change": round(percent_change, 2),
                "timeframe": timeframe,
                
                # Other prediction fields
                "token": token,
                "action": "BUY" if percent_change > 1 else "SELL" if percent_change < -1 else "HOLD",
                "entry_price": round(current_price, 6),
                "current_price": round(current_price, 6),
                "predicted_price": round(price, 6),
                "sentiment": "BULLISH" if percent_change > 1 else "BEARISH" if percent_change < -1 else "NEUTRAL",
                "rationale": f"Weighted average of technical, statistical, and machine learning models for {token} over {timeframe} timeframe.",
                "key_factors": ["Technical analysis", "Statistical models", "Machine learning prediction"],
                ...
                # Include inputs for traceability
                "inputs": {
                    "current_price": current_price,
            ...
            logger.logger.info(f"Generated weighted prediction for {token}: ${price:.6f} ({percent_change:+.2f}%)")
            return prediction
        ...
        try:
            ...
            # ================================================================
            # Fix nested structure by moving prediction fields to root level
            if "prediction" in llm_prediction and isinstance(llm_prediction["prediction"], dict):
                nested_pred = llm_prediction["prediction"]
                
                # Extract core prediction fields to root level
                for field in ["price", "confidence", "lower_bound", "upper_bound", "percent_change"]:
                    if field in nested_pred:
                        llm_prediction[field] = nested_pred[field]
                
                logger.logger.debug(f"🔄 Extracted nested prediction fields to root level for {token}")
        except Exception as e:
            logger.logger.warning(f"Error in LLM processing: {e} - falling back to combined prediction")
            # Fall back to combined prediction without LLM
        ...
        # Add other necessary fields for complete compatibility
        if "token" not in llm_prediction:
            llm_prediction["token"] = token
            
        if "action" not in llm_prediction:
            percent_change = llm_prediction.get("percent_change", 0)
            llm_prediction["action"] = "BUY" if percent_change > 1 else "SELL" if percent_change < -1 else "HOLD"
            
        if "entry_price" not in llm_prediction:
            llm_prediction["entry_price"] = current_price
            
        if "current_price" not in llm_prediction:
            llm_prediction["current_price"] = current_price
            
        if "predicted_price" not in llm_prediction and "price" in llm_prediction:
            llm_prediction["predicted_price"] = llm_prediction["price"]
        
        # Ensure all required fields exist
        for field in ["price", "confidence", "lower_bound", "upper_bound"]:
            if field not in llm_prediction:
                if field == "price":
                    llm_prediction[field] = round(current_price * 1.01, 6)
                elif field == "confidence":
                    llm_prediction[field] = 65.0
                elif field == "lower_bound":
                    llm_prediction[field] = round(llm_prediction.get("price", current_price) * 0.98, 6)
                elif field == "upper_bound":
                    llm_prediction[field] = round(llm_prediction.get("price", current_price) * 1.02, 6)
        
        # Log success
        price = llm_prediction.get("price", 0)
        percent_change = llm_prediction.get("percent_change", 0)
        logger.logger.info(f"Generated LLM prediction for {token}: ${price:.6f} ({percent_change:+.2f}%)")
...
"""
    ...
    def _parse_llm_response(self, response_text, token, current_price, 
                ...
                Ensures flat structure with database-compatible fields
                ...
                Returns:
                    Parsed LLM prediction result with flat database-compatible structure
                """
                # Try to parse as JSON
                ...
                # Create a clean result dictionary with flat structure
                clean_result = {}
                
                # Extract directly from nested prediction structure
                if "prediction" in result and isinstance(result["prediction"], dict):
                    prediction = result["prediction"]
                    
                    # Extract core prediction fields to root level
                    for field in ["price", "confidence", "lower_bound", "upper_bound", "percent_change", "timeframe"]:
                        if field in prediction:
                            clean_result[field] = prediction[field]
                    
                    # Validate numeric values
                    numeric_fields = ["price", "confidence", "lower_bound", "upper_bound", "percent_change"]
                    for field in numeric_fields:
                        if field in clean_result:
                            clean_result[field] = float(clean_result[field])
                
                # Copy non-nested fields directly
                for field in ["rationale", "sentiment", "key_factors"]:
                    if field in result:
                        clean_result[field] = result[field]
                
                # Add model weights used (maintains existing structure)
                clean_result["model_weights"] = {
                    "technical_analysis": 0.25,
                    "statistical_models": 0.25,
                    "machine_learning": 0.25,
                    "llm_enhanced": 0.25
                ...
                # Add model inputs (maintains existing structure)
                clean_result["inputs"] = {
                    "current_price": current_price,
                ...
                logger.logger.debug(f"✅ Parsed LLM response with flat database-compatible structure for {token}")
                ...
                if POLARS_AVAILABLE and pl is not None and len(raw_data) > 3:
                    try:
                        ...
                        if hasattr(self, 'db') and self.db:
                            ...
                            # For tokens without sparkline data, get from database
                            if hasattr(self, 'db') and self.db:
                                print(f"🔧 APPLYING SIMPLE DATABASE FIX")
                                result = self.db.force_database_supplementation(result, timeframe, token_id_map)
                            else:
                                print(f"❌ No database connection for supplementation")
                        else:
                            logger.logger.warning(f"❌ No database connection available for supplementation")
    ...
    def _generate_predictions(self, token: str, market_data: Dict[str, Any], timeframe: str = "1h") -> Dict[str, Any]:
        ...
        Uses price history arrays built from your 45,253 database records.
        NO FALLBACKS - Real data only, fail fast approach.
        
        Pipeline:
        1. Extract validated data from technical system
        2. Execute technical analysis with price history
        ...
        # Handle both flat structure (from our updated methods) and nested structure (from other sources)
        if not isinstance(market_data, dict):
            raise ValueError(f"Invalid market_data format: expected dict, got {type(market_data)}")

        # Check if market_data is flat structure (from updated trading bot methods)
        if 'token' in market_data and market_data.get('token') == token:
            # Flat structure from updated methods
            token_data = market_data
            logger.logger.debug(f"Using flat market data structure for {token}")
        elif token in market_data:
            # Nested structure (existing functionality)
            token_data = market_data[token]
            logger.logger.debug(f"Using nested market data structure for {token}")
        else:
            # Try to find token in market data by checking values (existing functionality)
            token_data = None
            ...
            if not token_data:
                available_tokens = list(market_data.keys())[:5]
        ...
        volume_24h = float(token_data.get('volume', 0) or token_data.get('total_volume', 0) or token_data.get('volume_24h', 0))
        market_cap = float(token_data.get('market_cap', 0))
        price_change_24h = float(token_data.get('price_change_percentage_24h', 0) or token_data.get('price_change_24h', 0))
        
        # Get price history from database instead of relying on external API data
        logger.logger.debug(f"Getting price history from database for {token}")
        ...
        if hasattr(self, 'db') and self.db:
            try:
                # Use database method to build price array from price_history table
                price_history = self.db.build_sparkline_from_price_history(token, hours=hours_lookback)
                if not price_history:
                    price_history = []
                data_source = 'database'
                logger.logger.debug(f"Retrieved {len(price_history)} price points from database for {token}")
            except Exception as db_error:
                logger.logger.warning(f"Database price history failed for {token}: {db_error}")
                # Fall back to checking if price history was provided in market_data
                price_history = token_data.get('sparkline', [])
                data_source = token_data.get('_sparkline_source', 'fallback')
        else:
            # No database available, check if price history was provided in market_data
            price_history = token_data.get('sparkline', [])
            data_source = token_data.get('_sparkline_source', 'provided')
        
        price_points = len(price_history) if price_history else 0
        
        logger.logger.debug(f"✅ Data extracted: ${current_price:.6f}, {price_points} price points from {data_source}")
        
        # ================================================================
        # 🧠 STEP 2: EXECUTE TECHNICAL ANALYSIS WITH PRICE HISTORY 🧠
        # ================================================================
        
        logger.logger.debug(f"Step 2/6: Executing technical analysis for {token}")
        
        if not price_history or len(price_history) < 10:
            raise ValueError(f"Insufficient price data for {token}: {price_points} points (minimum 10 required)")
        ...
        if not technical_analysis:
            raise ValueError(f"Technical analysis returned empty result for {token}")
        ...
        prediction = {
            ...
            'key_factors': [
                ...
                f"Data quality: {price_points} price points from {data_source}"
            ],
            'timestamp': datetime.now(),
            ...
            # Processing metadata
            'data_source': f'technical_system_validated_{data_source}',
            'price_points': price_points,
            'processing_time': round(time.time() - pipeline_start, 3),
            'pipeline_version': 'streamlined_v2.0'
        }
        ...
        prediction['pipeline_metadata'] = {
            ...
            'used_database_price_history': data_source == 'database',
            'timestamp': datetime.now().isoformat()
        }
