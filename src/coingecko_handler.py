...
class CoinGeckoHandler:
    ...
    def select_priority_tokens(self, market_data: List[Dict[str, Any]], max_tokens: int = 4, 
        ...
        Maximizes the value of limited API calls by choosing tokens with highest trading potential
        
        Args:
            market_data: List of market data from batch call
            max_tokens: Maximum number of priority tokens to select (free tier constraint)
        ...
        logger.logger.info(f"🎯 Selecting {max_tokens} priority tokens using '{selection_strategy}' strategy")
        ...
        for token in market_data:
            ...
            try:
                ...
                # Volume Score (0-100): Higher volume = more tradeable
                volume_score = min(100, (volume_24h / 1_000_000_000) * 20)  # $1B = 20 points
                
                # Momentum Score (0-100): Recent price movement momentum
                momentum_1h = abs(price_change_1h) * 10  # Amplify 1h movements
                momentum_24h = abs(price_change_24h) * 2  # Weight 24h movements  
                momentum_7d = abs(price_change_7d) * 0.5   # De-weight longer movements
                momentum_score = min(100, momentum_1h + momentum_24h + momentum_7d)
                
                # Volatility Score (0-100): Higher volatility = more opportunity
                ...
                # Market Cap Stability (0-100): Prefer established but not stagnant
                if market_cap_rank <= 10:
                    stability_score = 80  # Top 10 coins
                elif market_cap_rank <= 50:
                    stability_score = 60  # Top 50 coins
                elif market_cap_rank <= 100:
                    stability_score = 40  # Top 100 coins
                else:
                    stability_score = 20  # Lower ranked coins
                
                # Market Cap Liquidity (0-100): Ensure sufficient liquidity
                liquidity_score = min(100, (market_cap / 100_000_000) * 10)  # $100M = 10 points
                ...
                if selection_strategy == "volume":
                    # Pure volume-based selection
                    final_score = volume_score * 0.7 + liquidity_score * 0.3
                    
                elif selection_strategy == "momentum":
                    # Momentum-focused selection
                    final_score = momentum_score * 0.5 + volume_score * 0.3 + volatility_score * 0.2
                    
                elif selection_strategy == "volatility":
                    # Volatility-focused selection for swing trading
                    final_score = volatility_score * 0.4 + momentum_score * 0.3 + volume_score * 0.3
                    
                else:  # "adaptive" - balanced approach
                    # Adaptive scoring - balances all factors
                    final_score = (
                        volume_score * 0.25 +      # Trading volume
                        momentum_score * 0.25 +    # Recent momentum
                        volatility_score * 0.20 +  # Price volatility
                        stability_score * 0.15 +   # Market position
                        liquidity_score * 0.15     # Market cap liquidity
                    )
        ...
        # ================================================================
        # 🏆 STEP 4: SELECT TOP PRIORITY TOKENS
        # ================================================================
        ...
        # Select top tokens up to max_tokens limit
        selected_tokens = scored_tokens[:max_tokens]
        priority_token_ids = [token['token_id'] for token in selected_tokens]
        
        # ================================================================
        # 📊 STEP 5: LOG SELECTION RESULTS
        # ================================================================
        
        logger.logger.info(f"🎯 PRIORITY TOKEN SELECTION COMPLETE:")
        logger.logger.info(f"   Strategy: {selection_strategy}")
        ...
        for i, token in enumerate(selected_tokens, 1):
            logger.logger.info(
                ...
                f"Vola: {token['volatility_score']:.0f}"
            )
        ...
        selection_metadata = {
            ...
            'selection_details': selected_tokens
        }
    ...
    def _optimize_cache_for_trading(self, current_time: float, cache_duration_seconds: float) -> Dict[str, Any]:
        ...
        try:
            ...
            priority_tokens = self.select_priority_tokens(market_data=[], max_tokens=5, selection_strategy="adaptive")
    ...
    def find_token_id(self, token_symbol: str) -> Optional[str]:
        ...
        (Unchanged from original - already efficient)
        """
    ...
    def emergency_quota_conserve(self) -> None:
        ...
        self.quota_tracker.daily_limit = 200  # Very conservative
    ...
    def reset_to_normal_mode(self) -> None:
        ...
        self.min_request_interval = 60.0  # 60 seconds
        self.max_retries = 2
        self.quota_tracker.daily_limit = 400
    ...
    def __repr__(self) -> str:
        ...
        return (f"CoinGeckoHandler(base_url='{self.base_url}', "
                ...
                f"daily_limit={self.quota_tracker.daily_limit})")
