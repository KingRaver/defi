...
class CoinGeckoDataValidator:
    ...
    def validate_market_data_entry(entry: Dict[str, Any]) -> Tuple[bool, str]:
        """
        Validate a market data entry using TokenMappingManager for dynamic field detection
        
        This method uses TokenMappingManager to detect the data source and validate
        fields according to the expected format for that source.
        
        Returns:
            (is_valid, error_message)
        """
        try:
            # Import TokenMappingManager and config for dynamic validation
            from config import config
            
            # Use TokenMappingManager to detect what type of data this is
            source = config.detect_data_source(entry)
            
            # Handle different data source formats dynamically
            if source == 'coingecko':
                # This is a CoinGecko API response - validate CoinGecko fields
                field_mapping = config.get_field_mapping('coingecko')
                required_fields = ['id', 'symbol', 'current_price']  # Core CoinGecko fields
                
                for field in required_fields:
                    if field not in entry:
                        return False, f"Missing required CoinGecko field: {field}"
                
                # Validate price using existing method
                is_valid, _ = CoinGeckoDataValidator.validate_price_data(entry['current_price'])
                if not is_valid:
                    return False, "Invalid current_price"
                
                return True, "Valid CoinGecko market data"
                
            elif source == 'database':
                # This is database record - validate database fields  
                field_mapping = config.get_field_mapping('database')
                required_fields = ['chain', 'price']  # Core database fields
                
                for field in required_fields:
                    if field not in entry:
                        return False, f"Missing required database field: {field}"
                
                # Validate price from database format
                is_valid, _ = CoinGeckoDataValidator.validate_price_data(entry['price'])
                if not is_valid:
                    return False, "Invalid price in database record"
                
                return True, "Valid database record"
                
            elif source == 'unknown':
                # Unknown format - check if it's price history/chart data
                if 'prices' in entry and isinstance(entry.get('prices'), list):
                    # This is chart/price history data - validate chart structure
                    prices_data = entry.get('prices', [])
                    if not prices_data or len(prices_data) == 0:
                        return False, "Chart prices array is empty"
                    
                    # Validate first few price points
                    for i, price_point in enumerate(prices_data[:3]):
                        if not isinstance(price_point, list) or len(price_point) < 2:
                            return False, f"Invalid price point structure at index {i}"
                        
                        try:
                            price_value = float(price_point[1])
                            if price_value <= 0 or math.isnan(price_value) or math.isinf(price_value):
                                return False, f"Invalid price value at index {i}: {price_value}"
                        except (ValueError, TypeError, IndexError):
                            return False, f"Invalid price value format at index {i}"
                    
                    return True, "Valid chart/price history data"
                
                # Check if it's other valid response types
                elif isinstance(entry, dict) and len(entry) > 0:
                    # Basic validation for other response types
                    return True, "Valid response data"
                else:
                    return False, "Unknown data format"
            
            else:
                # Handle other detected sources  
                if isinstance(entry, dict) and len(entry) > 0:
                    return True, f"Valid {source} data"
                else:
                    return False, f"Invalid {source} data format"
            
        except Exception as e:
            # Fallback to basic validation if TokenMappingManager fails
            try:
                # FALLBACK: Check if it's chart data (price history)
                if 'prices' in entry and isinstance(entry.get('prices'), list):
                    prices_data = entry.get('prices', [])
                    if prices_data and len(prices_data) > 0:
                        # Quick validation of first price point
                        first_point = prices_data[0]
                        if isinstance(first_point, list) and len(first_point) >= 2:
                            price_value = float(first_point[1])
                            if price_value > 0 and not math.isnan(price_value):
                                return True, "Valid chart data (fallback validation)"
                    
                    return False, "Invalid chart data structure"
                
                # FALLBACK: Check if it's standard market data
                elif 'current_price' in entry:
                    price = float(entry['current_price'])
                    if price > 0 and not math.isnan(price):
                        return True, "Valid market data (fallback validation)"
                    else:
                        return False, "Invalid price in market data"
                
                # FALLBACK: Basic dictionary validation
                elif isinstance(entry, dict) and len(entry) > 0:
                    return True, "Valid response data (fallback validation)"
                
                else:
                    return False, f"Validation error with fallback: {str(e)}"
                    
            except Exception as fallback_error:
                return False, f"Validation error: {str(e)}, Fallback error: {str(fallback_error)}"
...
class CoinGeckoQuotaTracker:
    ...
    def can_make_request(self) -> Tuple[bool, str]:
        ...
        if time_since_last < 30.0:  # 30-second minimum
            return False, f"Rate limit: need to wait {30.0 - time_since_last:.1f}s"
...
class CoinGeckoHandler:
    ...
    def get_market_data(self, params: Optional[Dict[str, Any]] = None, timeframe: str = "24h", 
            ...
            import inspect
            caller_info = inspect.stack()[1] if len(inspect.stack()) > 1 else None
            logger.logger.info(f"🔍 DEBUG: Caller filename: {caller_info.filename if caller_info else 'None'}")
            logger.logger.info(f"🔍 DEBUG: Contains 'api_manager': {'api_manager' in str(caller_info.filename).lower() if caller_info else False}")
            endpoint = "coins/markets"
            
            # ================================================================
            # 🎯 STEP 0: DYNAMIC TOKEN SELECTION WITH TOKENMAPPINGMANAGER
            # ================================================================
            
            # Get dynamic token list using TokenMappingManager first
            ...
            try:
                ...
                if all_tokens_info and all_tokens_info.get('all_unique_symbols'):
                    ...
                    if api_ids:
                        ...
                        logger.logger.info(f"✅ TokenMappingManager: Using {len(api_ids)} dynamic tokens")
                    else:
                        logger.logger.warning("⚠️ TokenMappingManager: No CoinGecko IDs found for available tokens")
                else:
                    logger.logger.warning("⚠️ TokenMappingManager: No available tokens found")
            except Exception as tmm_error:
                logger.logger.warning(f"⚠️ TokenMappingManager failed: {str(tmm_error)}")
                ...
                try:
                    ...
                    if db_results:
                        ...
                        if db_token_ids:
                            ...
                            logger.logger.info(f"✅ Database fallback: Using {len(db_token_ids)} tokens by market cap")
                    
                except Exception as db_error:
                    logger.logger.warning(f"⚠️ Database fallback failed: {str(db_error)}")
            
            # Last resort: hybrid method using cache or minimal API call
            if not dynamic_token_ids:
                logger.logger.warning("⚠️ Using hybrid method as last resort")
                try:
                    # Try to get cached token list
                    cache_key = "active_token_ids_market_cap"
                    cached_tokens = self._get_from_cache(cache_key)
                    if cached_tokens and isinstance(cached_tokens, list):
                        dynamic_token_ids = ",".join(cached_tokens[:30])
                        logger.logger.info(f"✅ Cache fallback: Using {len(cached_tokens[:30])} cached tokens")
                    else:
                        # Minimal API call to get current top tokens
                        logger.logger.info("🔄 Making minimal API call to get current top tokens")
                        minimal_params = {
                            "vs_currency": "usd",
                        ...
                        minimal_result = self.get_with_cache("coins/markets", minimal_params)
                        if minimal_result and isinstance(minimal_result, list):
                            token_ids = [token.get('id') for token in minimal_result if token.get('id')]
                            if token_ids:
                                dynamic_token_ids = ",".join(token_ids)
                                # Cache for future use
                                self._add_to_cache(cache_key, token_ids)
                                logger.logger.info(f"✅ Hybrid method: Retrieved {len(token_ids)} tokens via API")
                except Exception as hybrid_error:
                    logger.logger.error(f"❌ Hybrid method failed: {str(hybrid_error)}")
                    return None
            
            # ================================================================
            # 🚀 STEP 1: SET PARAMETERS WITH DYNAMIC TOKEN LIST
            # ================================================================
            ...
            if params is None:
                ...
                params = {
                    "vs_currency": "usd",
                    "ids": dynamic_token_ids,  # 🎯 USING DYNAMIC TOKEN LIST
                    "order": "market_cap_desc",
                    "per_page": 30,
                    "page": 1,
                ...
                # If params provided, use dynamic tokens if 'ids' not specified
                if 'ids' not in params and dynamic_token_ids:
                    params['ids'] = dynamic_token_ids
                    logger.logger.info("✅ Applied dynamic token list to provided params")
            ...
            # ================================================================
            # 🚀 STEP 2: GET BATCH BASIC DATA (EFFICIENT)
            # ================================================================
            ...
            # ================================================================
            # 🎯 STEP 3: DYNAMIC PRIORITY TOKENS WITH TOKENMAPPINGMANAGER
            # ================================================================
            
            # If priority_tokens not provided, get them dynamically using TokenMappingManager
            if priority_tokens is None:
                try:
                    from config import TokenMappingManager
                    token_mapper = TokenMappingManager()
                    
                    # Get available tokens and convert to CoinGecko IDs for priority selection
                    all_tokens_info = token_mapper.get_all_available_tokens(include_database=True)
                    if all_tokens_info and all_tokens_info.get('all_unique_symbols'):
                        # Get top 4 symbols as priority tokens
                        priority_symbols = all_tokens_info['all_unique_symbols'][:4]
                        ...
                        for symbol in priority_symbols:
                            coingecko_id = token_mapper.symbol_to_coingecko_id(symbol)
                            if coingecko_id:
                                priority_tokens.append(coingecko_id)
                        
                        logger.logger.info(f"✅ TokenMappingManager: Dynamic priority tokens: {priority_tokens}")
                    else:
                        logger.logger.warning("⚠️ TokenMappingManager: No priority tokens found")
                        
                except Exception as tmm_priority_error:
                    logger.logger.warning(f"⚠️ TokenMappingManager priority tokens failed: {str(tmm_priority_error)}")
            
            # Database fallback for priority tokens if TokenMappingManager fails
            if priority_tokens is None:
                try:
                    from database import CryptoDatabase
                    db = CryptoDatabase()
                    ...
                    # Get top 4 tokens by market cap from database with proper aggregation
                    cursor.execute("""
                        SELECT DISTINCT coin_id, MAX(market_cap) as max_market_cap
                        FROM coingecko_market_data 
                        WHERE market_cap > 0 AND coin_id IS NOT NULL
                    ...
                    db_priority_results = cursor.fetchall()
                    if db_priority_results:
                        priority_tokens = [row['coin_id'] for row in db_priority_results if row['coin_id']]
                        logger.logger.info(f"✅ Database fallback: Dynamic priority tokens: {priority_tokens}")
                        
                except Exception as db_priority_error:
                    logger.logger.warning(f"⚠️ Database priority tokens fallback failed: {str(db_priority_error)}")
            
            # Cache fallback for priority tokens as last resort
            if priority_tokens is None:
                try:
                    cache_key = "priority_token_ids_market_cap"
                    cached_priority = self._get_from_cache(cache_key)
                    if cached_priority and isinstance(cached_priority, list):
                        priority_tokens = cached_priority[:4]
                        logger.logger.info(f"✅ Cache fallback: Priority tokens: {priority_tokens}")
                    else:
                        # Extract top 4 from current result as last resort
                        if result and len(result) >= 4:
                            priority_tokens = [token.get('id') for token in result[:4] if token.get('id')]
                            # Cache for future use
                            self._add_to_cache(cache_key, priority_tokens)
                            logger.logger.info(f"✅ Hybrid fallback: Priority tokens from current result: {priority_tokens}")
                        
                except Exception as cache_priority_error:
                    logger.logger.warning(f"⚠️ Cache priority tokens fallback failed: {str(cache_priority_error)}")
            
            # ================================================================
            # 🎯 STEP 4: ENHANCE PRIORITY TOKENS WITH PRICE HISTORY (SELECTIVE)
            # ================================================================
            ...
            if include_price_history and priority_tokens:
                ...
                logger.logger.info("📊 CoinGecko: Direct price history fetch using CoinGecko API")
                
                # 🎯 DIRECT PRICE HISTORY FETCH: No delegation, handle internally
                enhanced_result = []
                api_calls_made = 0
                        ...
                        try:
                            ...
                            if chart_data and isinstance(chart_data, dict):
                                ...
                                if prices_data and len(prices_data) >= 20:  # Minimum viable for technical analysis
                                    ...
                                    token_data['_history_source'] = 'coingecko_direct'
                ...
                logger.logger.info(f"🎯 Enhanced {api_calls_made} priority tokens with direct CoinGecko price history")

            else:
                # No price history requested
                for token_data in result:
                    if isinstance(token_data, dict):
                        token_data['_enhanced_with_history'] = False

            # ================================================================
            # 🏷️ STEP 5: ADD PROCESSING METADATA
            # ================================================================
            ...
            processing_metadata = {
                ...
                '_processing_approach': 'tokenmapping_manager_hybrid',
                ...
                '_dynamic_token_source': 'tokenmapping_manager' if dynamic_token_ids else 'fallback'
            }
            ...
            logger.logger.info(f"✅ Returning {len(result)} validated market data entries with TokenMappingManager enhancement")
            return result
    ...
    def get_multiple_tokens_by_symbol(self, symbols: List[str]) -> Dict[str, str]:
            """
            Get CoinGecko IDs for multiple token symbols using TokenMappingManager
            (Enhanced with quota checking and database-first approach)
            """
            # Initialize result and tracking
            result = {}
            symbols_to_fetch = []
            
            # Try TokenMappingManager first for all symbols
            try:
                from config import TokenMappingManager
                token_mapper = TokenMappingManager()
                logger.logger.debug("✅ Using TokenMappingManager for batch symbol lookup")
                
                for symbol in symbols:
                    symbol_upper = symbol.upper()
                    
                    # PRIMARY: Check TokenMappingManager database lookup
                    try:
                        coingecko_id = token_mapper.database_lookup_symbol_to_coingecko_id(symbol_upper)
                        if coingecko_id:
                            result[symbol] = coingecko_id
                            logger.logger.debug(f"✅ Database lookup: {symbol} -> {coingecko_id}")
                            continue
                    except Exception as db_lookup_error:
                        logger.logger.debug(f"Database lookup failed for {symbol}: {str(db_lookup_error)}")
                    
                    # SECONDARY: Check TokenMappingManager comprehensive lookup
                    try:
                        coingecko_id = token_mapper.symbol_to_coingecko_id(symbol_upper)
                        if coingecko_id:
                            result[symbol] = coingecko_id
                            logger.logger.debug(f"✅ TMM lookup: {symbol} -> {coingecko_id}")
                            continue
                    except Exception as tmm_lookup_error:
                        logger.logger.debug(f"TMM lookup failed for {symbol}: {str(tmm_lookup_error)}")
                    
                    # TERTIARY: Check cache
                    symbol_lower = symbol.lower()
                    if symbol_lower in self.token_id_cache:
                        result[symbol] = self.token_id_cache[symbol_lower]
                        logger.logger.debug(f"✅ Cache hit: {symbol} -> {self.token_id_cache[symbol_lower]}")
                        continue
                ...
                logger.logger.warning(f"⚠️ TokenMappingManager unavailable: {str(tmm_error)}")
                # Fallback to cache-only approach
                for symbol in symbols:
                    symbol_lower = symbol.lower()
                    if symbol_lower in self.token_id_cache:
                        result[symbol] = self.token_id_cache[symbol_lower]
                        logger.logger.debug(f"✅ Cache fallback: {symbol} -> {self.token_id_cache[symbol_lower]}")
                    else:
                        symbols_to_fetch.append(symbol)
            
            if not symbols_to_fetch:
                logger.logger.info(f"✅ All {len(symbols)} symbols resolved without API call")
                return result
            
            logger.logger.info(f"🔍 Need to fetch {len(symbols_to_fetch)} symbols from API: {symbols_to_fetch}")
            ...
            if quota_status['daily_remaining'] <= 5:
                ...
                # Return partial results with None for unfound symbols
                for symbol in symbols_to_fetch:
                    result[symbol] = None
                return result
            
            # Try database fallback before API call
            if symbols_to_fetch:
                try:
                    from database import CryptoDatabase
                    db = CryptoDatabase()
                    ...
                    # Batch database lookup for remaining symbols
                    placeholders = ",".join(["?" for _ in symbols_to_fetch])
                    upper_symbols = [s.upper() for s in symbols_to_fetch]
                    ...
                    for row in db_results:
                        if row['symbol'] and row['coin_id']:
                            db_found[row['symbol'].upper()] = row['coin_id']
                    
                    # Apply database results
                    remaining_symbols = []
                    for symbol in symbols_to_fetch:
                        symbol_upper = symbol.upper()
                        if symbol_upper in db_found:
                            result[symbol] = db_found[symbol_upper]
                            # Update cache
                            self.token_id_cache[symbol.lower()] = db_found[symbol_upper]
                            logger.logger.debug(f"✅ Database batch lookup: {symbol} -> {db_found[symbol_upper]}")
                        else:
                            remaining_symbols.append(symbol)
                    
                    symbols_to_fetch = remaining_symbols
                    logger.logger.info(f"✅ Database resolved {len(db_found)} additional symbols")
                    
                except Exception as db_batch_error:
                    logger.logger.warning(f"⚠️ Database batch lookup failed: {str(db_batch_error)}")
            
            # If still have symbols to fetch, use API
            if not symbols_to_fetch:
                logger.logger.info(f"✅ All symbols resolved without API call (via database)")
                return result
            
            logger.logger.info(f"🌐 Final API fetch needed for {len(symbols_to_fetch)} symbols")
            
            # Fetch coins list once for all remaining symbols
            endpoint = "coins/list"
            coins_list = self.get_with_cache(endpoint)
            ...
            if not coins_list:
                ...
                for symbol in symbols_to_fetch:
                    result[symbol] = None
                    logger.logger.warning(f"❌ Could not fetch coin list - setting {symbol} to None")
                return result
            
            # Create lookup dictionary from coins list
            symbol_to_id = {}
            for coin in coins_list:
                coin_symbol = coin.get('symbol', '').upper()  # Normalize to uppercase
            ...
            for symbol in symbols_to_fetch:
                ...
                if symbol_upper in symbol_to_id:
                    result[symbol] = symbol_to_id[symbol_upper]
                    self.token_id_cache[symbol.lower()] = symbol_to_id[symbol_upper]
                    logger.logger.debug(f"✅ API lookup: {symbol} -> {symbol_to_id[symbol_upper]}")
                else:
                    logger.logger.warning(f"❌ Could not find ID for {symbol}")
                    result[symbol] = None
            
            # Final summary
            successful = len([v for v in result.values() if v is not None])
            logger.logger.info(f"✅ Batch lookup complete: {successful}/{len(symbols)} symbols resolved")
